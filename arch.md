...
✅ 2026-08-19 재갱신 — **회원탈퇴 정책을 소프트 삭제(익명화)에서 즉시 하드 삭제로 전면 교체 확정 및 구현 완료**(WITHDRAW-1~4). 기존 "7일 유예기간+자동복구+익명화 스케줄러" 구조를 전부 폐지하고, 아래 "탈퇴 정책(2026-08-19 확정)" 절로 대체한다. 이 절 이전의 "7일 유예기간/자동복구/익명화" 관련 서술은 전부 낡은 내용이므로 새 정책을 따른다. **상세 원본은 `docs/collab/user.md`("회원정보·개인정보 보유·탈퇴·파기 정책") — 이 문서가 source of truth이고, 아래 표는 그 요약이다.** 원본과 이 요약이 다르면 원본을 따른다.

### 탈퇴 정책 (2026-08-19 확정 및 구현 완료 — WITHDRAW-1~4, `docs/collab/user.md` §19)
...
**개인정보처리방침 고지 보유기간 전체 표는 `docs/collab/user.md` §1.1 참고**(본인확인정보=즉시파기, 신청정보/결제정보=상품수령후6개월, 상담정보=상담일로부터6개월, 회원가입정보=상품수령후6개월).

✅ **"회원가입 정보 보유기간" 정합성 — 2026-08-19 해소(`docs/collab/user.md` §17.1)**: "상품 수령 후 6개월"은 **탈퇴하지 않고 계속 활동하는 회원**에게 적용되는 기본 보유기간(만료 시 파기 대상)이고, **회원탈퇴 시에는 이 기간을 기다리지 않고 지체 없이 파기한다** — 탈퇴 즉시 `User` 하드 삭제 정책이 이 보유기간 상한선보다 항상 먼저 적용되는 별도 트리거다. 상품을 신청/수령한 적 없는 회원은 이 6개월 타이머가 애초에 시작되지 않을 뿐, 탈퇴하면 언제든 즉시 파기된다는 원칙은 동일하다. 그 외 §17.2(비밀번호 제3자 제공 문구 오류 의심), §17.3(제3자 제공과 처리위탁 조항 혼재)은 여전히 방침 문안 자체의 확인이 필요한 사항으로 `docs/collab/user.md`에 남아있다.
