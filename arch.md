...
⚠️ **미해결 — 개인정보처리방침 문구와의 정합성 확인 필요(`docs/collab/user.md` §17 인용)**: 현재 전달받은 개인정보처리방침 문안에 "회원가입 정보 보유기간: 상품 수령 후 6개월"이라는 문구가 있는데, 상품을 신청/수령한 적 없는 회원은 기준일 자체가 없다. 이번에 확정한 "탈퇴 즉시 `User` 하드 삭제" 정책과 이 문구를 어떻게 정리할지(예: 회원가입 정보는 계정 존속 자체가 기준이고, "상품 수령 후 6개월"은 신청 관련 개인정보에만 적용되는 문구였다는 식으로 명확화)는 아직 정리되지 않았다 — 법무/개인정보 담당자 확인 필요, 임의로 해석해 구현하지 않는다. 그 외에도 §17.2(비밀번호 제3자 제공 문구 오류 의심), §17.3(제3자 제공과 처리위탁 조항 혼재) 등 방침 문안 자체의 확인 필요 사항이 `docs/collab/user.md`에 별도로 정리돼 있다.
