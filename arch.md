...
✅ 2026-08-19 재갱신 — **회원탈퇴 정책을 소프트 삭제(익명화)에서 즉시 하드 삭제로 전면 교체 확정**(구현은 아직, 정책만 확정). 기존 "7일 유예기간+자동복구+익명화 스케줄러" 구조를 전부 폐지하고, 아래 "탈퇴 정책(2026-08-19 확정)" 절로 대체한다. 이 절 이전의 "7일 유예기간/자동복구/익명화" 관련 서술은 전부 낡은 내용이므로 새 정책을 따른다. **상세 원본은 `docs/collab/user.md`("회원정보·개인정보 보유·탈퇴·파기 정책") — 이 문서가 source of truth이고, 아래 표는 그 요약이다.** 원본과 이 요약이 다르면 원본을 따른다.

### 탈퇴 정책 (2026-08-19 확정 — 구현 전, 스코프 분석 후 정책만 확정)

| 항목 | 정책 |
|---|---|
| 탈퇴 효력 | 즉시(유예기간 없음) |
| 7일 복구·자동 복구 | 폐지 |
| `User` row | 탈퇴 처리 과정에서 즉시 하드 삭제(물리 삭제) — 별도 배치 스케줄러로 미루지 않는다 |
| Access Token | 즉시 무효화(블랙리스트) |
| `RefreshTokenSession` | `revoke` 상태로 남기지 않고 즉시 하드 삭제(토큰 문자열 자체를 남기지 않기 위함) |
| `ApplicationDailyLimit` | 삭제(업무 이력·거래 증빙이 아니라 순수 운영 카운터라서) |
| `Application`/`Applicant`/`ApplicationMember`/`Receiver` | 유지 — 개인정보처리방침 고지 기준 **상품 수령일로부터 6개월**(회원탈퇴 시점과 무관하게 별도 기산). 전자상거래법상 거래기록 법정 보존기간이 더 길게 적용될 수 있어 최종 확정 전(`docs/collab/user.md` §17.4) 자동삭제 스케줄러는 아직 구현하지 않는다 |
| 결제·거래 기록(무통장입금 등) | 유지 — 위와 동일하게 상품 수령 후 6개월(자체 고지) + 법정 보존기간 확인 후 확정, 그때까지 자동삭제 스케줄러 미구현 |
| `Inquiry` | 유지 — 상담정보 보존정책(`docs/specs/inquiry/requirements.md` §⑧, 접수일`createdAt` 기준 6개월)에 따라 별도 관리 |
| `Review` | 작성자 `User` 계정은 삭제하되 **본문은 유지**(회원탈퇴로 자동 삭제하지 않음). 작성자 표시명이 개인정보에 해당한다고 판단되면 탈퇴 후 익명화할 수 있음(권장, 아직 확정 아님) — 최종 삭제·익명화 정책은 콘텐츠 운영정책으로 별도 확정 |
| `Board.created_by_user_id` | 유지(관리자 작성 이력) |
| `AdminActivityLog.admin_id` | 유지 — 관리자 계정이 삭제되어도 감사기록의 `adminId`는 임의로 변경·삭제하지 않는다(감사로그 무결성이 개인정보 삭제보다 우선). 단 감사로그에 관리자 이름·이메일 등 별도 개인정보를 저장한다면 그 항목의 보존기간은 별도로 정의해야 함 |
| 법정대리인 개인정보(만 14세 미만 아동 관련, 현재 프로젝트에 해당 수집 없음) | 조건부 정책만 기록: 동의 정상 확인 시 아동 정보 파기 시점까지 보관, 동의 거부·미확인 시 5일 이내 파기. `User` 탈퇴 여부만으로 일괄 삭제하지 않고 해당 아동 정보의 보유·파기 정책과 연결 관리 |
| 동일 이메일 재가입 | 가능 — 완전 신규 `User` row로 생성되며 과거 데이터(신청/문의/후기 등 `userId` 기준 소유권 전부)는 자동 승계하지 않는다 |
| 탈퇴 후 로그인 | 불가(row 자체가 없음) |
| 개인정보 익명화 스케줄러 | 제거(하드 삭제라 익명화 대상 자체가 없음) |
| 거래·상담 등 보유기간 만료 데이터 파기 배치 | 회원탈퇴 처리와 **완전히 별개의 인프라**로 설계 — 데이터 종류별 보유기간(`Inquiry: createdAt+6개월`, `Application/결제: 상품 수령일+확정 보유기간` 등) 기준. 파기 시 복구 불가능하게 실제 삭제하고, 필요 시 파기 유형/건수/일시/결과만 감사 로그로 남기며 **파기된 개인정보 원문을 로그에 다시 저장하지 않는다** |

**삭제 범위 원칙**: 탈퇴 시 `User`와 인증·계정에 종속된 운영 데이터(`RefreshTokenSession`, `ApplicationDailyLimit`)만 하드 삭제한다. `Application`·`Inquiry`·결제·업무 이력은 함께 삭제하지 않고 각 데이터 자체의 보존기간 정책을 따른다 — `arch.md` §5.1(모듈 간 영속 참조는 FK 없는 `Long id`)이 이미 전역 적용돼 있어 `User` row가 사라져도 이 도메인들의 참조 무결성 문제는 없다(스냅샷 저장 패턴 덕분에 화면 표시도 그대로 동작함, `Applicant`/`authorDisplayName`/`Inquiry.name`·`email`·`phone` 참고). 회원 계정의 존속 여부와 업무 개인정보의 보존 여부는 완전히 분리된 별개의 생명주기다(`docs/collab/user.md` §1).

**개인정보처리방침 고지 보유기간 전체 표는 `docs/collab/user.md` §1.1 참고**(본인확인정보=즉시파기, 신청정보/결제정보=상품수령후6개월, 상담정보=상담일로부터6개월, 회원가입정보=문서상 상품수령후6개월 — 2026-08-19 문구 확인 완료).

⚠️ **미해결 — "회원가입 정보 보유기간" 문구와 무주문 회원의 정합성(`docs/collab/user.md` §17.1)**: "회원가입 정보 보유기간: 상품 수령 후 6개월"이라는 문구 자체는 확인됐으나(오탈자 아님), 상품을 신청/수령한 적 없는 회원은 기산점 자체가 없다는 문제가 남아있다. 이번에 확정한 "탈퇴 즉시 `User` 하드 삭제"는 이 상한선과 별개의 트리거로, 실제 탈퇴한 회원에게는 "6개월"이 적용될 일이 없다 — 문제가 되는 건 **탈퇴하지 않고 계속 활동하는 무주문 회원**의 계정 정보에 이 상한선을 어떻게 적용할지다. 법무/개인정보 담당자 확인 필요, 임의로 해석해 구현하지 않는다. 그 외에도 §17.2(비밀번호 제3자 제공 문구 오류 의심), §17.3(제3자 제공과 처리위탁 조항 혼재) 등 방침 문안 자체의 확인 필요 사항이 `docs/collab/user.md`에 별도로 정리돼 있다.
...
- 활성 사용자 여부 검증
- 약관 동의 여부 검증
- 탈퇴 요청(즉시 하드 삭제, 복구 없음 — 위 "탈퇴 정책" 참고)
- 관리자 역할 검증
...
- 탈퇴는 즉시 확정되며 유예기간·복구가 없다 — `User` row를 하드 삭제한다(2026-08-19 정책 변경, 위 "탈퇴 정책" 표 참고). `Application`/`Inquiry`/결제 등 다른 도메인의 이력은 각자의 보존정책에 따라 별도로 유지된다.
...
- 계정없음/비밀번호불일치/OAuth전용계정은 로그인 응답에서 전부 동일한 오류로 처리한다 — 이메일 존재 여부가 응답 차이로 새어나가지 않게 하기 위함이다(§8.4의 principal 규칙과 별개로, 이건 "타인 계정 존재 여부 비노출" 원칙). "탈퇴유예기간경과" 케이스는 정책 변경으로 소멸(탈퇴 즉시 row가 없으므로 애초에 로그인 시도 자체가 계정없음과 동일하게 처리됨).
...
- **회원탈퇴해도 후기 본문은 유지한다(2026-08-19 확정, `docs/collab/user.md` §10)**: `User` 탈퇴 정책이 하드 삭제로 바뀌어도(§4.1) `Review` 본문은 함께 삭제하지 않는다 — `author_display_name`이 이미 `User.name`과 독립된 스냅샷이라 표시에 영향이 없고, 콘텐츠 자체를 삭제 대상으로 볼 이유가 없다는 판단. 탈퇴한 작성자 본인은 이후 수정·삭제를 할 수 없다(로그인 자체가 불가하므로). `Review.user_id`는 과거 작성자 식별값으로 그대로 유지한다. **미확정**: 작성자 표시명(`author_display_name`)이 개인정보에 해당한다고 판단될 경우 탈퇴 후 익명화할지는 권장 사항일 뿐 아직 확정하지 않았다 — 콘텐츠 운영정책으로 별도 결정 필요.
...
- ~~탈퇴 요청 후 7일이 지난 User 개인정보 익명화~~ — 2026-08-19 정책 변경으로 소멸: 탈퇴가 즉시 하드 삭제로 바뀌어 유예기간·익명화 대상 자체가 없다(§4.1 "탈퇴 정책" 참고)
- 고아 임시 파일 정리
- 실패한 파일 보상 삭제 재처리
- (신규, 별도 후속 구현) 거래·상담 등 개인정보 보유기간이 지난 데이터 파기 배치 — 예: `Inquiry`는 접수일(`createdAt`)+6개월(`docs/specs/inquiry/requirements.md` §⑧), `Application`/결제 이력은 상품 수령일+확정 보유기간(현재 방침 고지는 6개월이나 법정 보존기간 확인 전이라 미구현, §4.1 참고). 회원탈퇴 처리(즉시 하드 삭제)와는 **완전히 별개의 인프라**로 설계한다(`docs/collab/user.md` §14) — 이번 탈퇴 정책 변경과 독립된 별도 작업. 파기는 복구 불가능하게 실제 삭제하고, 필요 시 파기 유형/건수/일시/결과만 감사 로그로 남기며 파기 대상 개인정보 원문은 로그에 다시 저장하지 않는다.
