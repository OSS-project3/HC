...
> 이 문서는 `DB.md`와 `API-명세.md`를 기준으로 모놀리식 백엔드의 패키지 구조와 내부 모듈 규칙을 정의한다.
> 구현 코드와 본 문서가 충돌하면 API 계약은 `API-명세.md`, 데이터 구조와 제약은 `DB.md`를 우선 확인한다.
...
├──────────────────────────────────────────┤
│ Application 계층                        │
│ UseCase Service · Transaction · 권한검사 │
├──────────────────────────────────────────┤
│ Domain 계층                             │
│ Aggregate · Entity · VO · 상태전이·정책  │
├──────────────────────────────────────────┤
│ Infrastructure 계층                     │
│ JPA · S3 · Redis · JWT · 이미지 렌더러   │
└──────────────────────────────────────────┘
          │
          ├── RDBMS
          ├── Object Storage
...
- 패키지는 기술이 아니라 도메인을 최상위 기준으로 나눈다.
- 각 도메인 내부에서는 API/Application/Domain/Infrastructure 책임을 구분한다.
- 도메인 간 호출은 공개된 Application Service 또는 Port를 통한다.
...
Controller
  → Request DTO 검증
  → UseCase Service 호출
  → Aggregate 조회 및 도메인 규칙 실행
  → Repository 저장
  → Response DTO 변환
  → 공통 응답 반환
```

Controller는 HTTP를 해석하고, Application Service는 유스케이스를 조정하며, Entity는 자신의 상태와 불변조건을 지킨다.
...
모든 도메인에 네 개의 하위 패키지를 기계적으로 만들 필요는 없다. 파일이 실제로 존재할 때만 만든다. 빈 계층을 유지하기 위한 빈 클래스나 인터페이스는 만들지 않는다.
...
| 계층 | 책임 | 금지 사항 |
|---|---|---|
| `api` | HTTP 매핑, 인증 principal 수신, DTO 검증, 응답 코드 결정 | 비즈니스 상태 변경, Repository 직접 호출 |
| `application` | 유스케이스 조정, 트랜잭션, 권한·소유권 검사, 도메인 간 협력 | HTTP 객체 의존, JPA 세부사항 노출 |
| `domain` | 상태 전이, 불변조건, 계산 정책, Entity/VO, Repository 계약 | Spring MVC, S3/JWT 등 외부 기술 의존 |
| `infrastructure` | JPA 구현, 외부 저장소, Redis, 이미지 처리, 기술 어댑터 | 업무 상태 전이 결정 |
...
- `totalPrice = 신청 당시 CardType.price × totalQuantity`다.
- 이후 카드 기준 가격이 바뀌어도 기존 신청 금액은 변경하지 않는다.
...
4. 다른 모듈의 Repository를 직접 호출하는 대신 공개 Application Service 또는 Port를 사용한다.
5. 조회 화면에서 여러 모듈 데이터를 조합할 때는 전용 Query Service를 사용한다.
...
- `domain`은 `api`, `infrastructure`를 참조하지 않는다.
- `application`은 Spring MVC 타입을 참조하지 않는다.
- `infrastructure`는 domain/application이 정의한 인터페이스를 구현한다.
...
| Application | Card | 카드 종류 활성 여부·현재 가격 조회 |
...
## 6. Command와 Query 분리 규칙

물리적으로 별도 서버나 데이터베이스를 사용하는 CQRS는 적용하지 않는다. 다만 코드 책임은 다음처럼 구분한다.
...
Command는 Aggregate를 조회하고 의미 있는 도메인 메서드로 변경한다.
...
Query는 화면에 필요한 projection을 직접 조회할 수 있다. 조회 성능을 위해 모든 Aggregate를 메모리에 로딩하지 않는다.
...
- API 필드명과 enum 값은 `API-명세.md`를 그대로 따른다.
...
- `PAYMENT_PENDING` 상태로 3영업일이 지난 신청 자동 취소
...
- `API-명세.md`의 HTTP status와 응답 필드
...
아래 항목은 `DB.md`와 `API-명세.md`에서도 확정되지 않았다. 구현 전 기획 결정을 받아야 한다.
...
- API 요청·응답·HTTP status 변경은 먼저 `API-명세.md`에 반영한다.
