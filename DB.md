...
> Application 데이터 모델은 [`docs/specs/application/data-model.md`](docs/specs/application/data-model.md)로 이전되었습니다.
...
**가격 정책 (2026-08-01 정정)**
- 결제 금액은 카드 종류별 고정값을 하드코딩하거나 `CardType.price × total_quantity`로 자동 계산하지 않는다.
- `Application.total_price` = **신청 건별 사전 상담을 통해 확정된 최종 결제 금액**. 확정 이후에는 기존 신청 금액을 자동으로 변경하지 않는다.
- `CardType.price`는 신청 결제 금액의 자동 계산 기준으로 사용하지 않는다. 컬럼 유지 여부와 용도는 [TBD]다.
- `total_price`는 상담 확정 금액을 저장하므로 **NOT NULL 유지**한다.
- ✅ 결제 확인 상태는 `Application.payment_status` 하나로만 관리(위 2.5절 참고, `Payment.is_confirmed`는 제거)

**입금 및 환불 정책 (2026-08-01 확정)**
- 입금 기한은 신청일로부터 3일 이내다. 기한 내 미입금 시 `CANCELLED` 처리한다.
- `RECEIVED`: 전액 환불 가능 — 아직 제작 전
- `REVIEWING`: 전액 환불 가능 — 검토 중
- `PHOTO_REJECTED`: 전액 환불 가능 — 사용자 재업로드 또는 취소 가능
- `PAYMENT_PENDING`은 미입금 상태이므로 환불이 아니라 취소 처리한다.
- `NAME_EDITING` 이후 환불 정책은 [TBD]다.
...
| 결제 금액 | `Application.total_price`에 신청 건별 사전 상담 확정 금액을 저장. `CardType.price`로 자동 계산하거나 하드코딩하지 않으며, `CardType.price` 컬럼의 유지 여부와 용도는 [TBD] |
...
- **refresh 토큰 rotation용 세션 저장소** — DB 테이블(`refresh_token_sessions`)로 유지할지 Redis만 쓸지 미정, 구현 단계에서 결정 가능 (`docs/api/README.md` User 도메인 TODO와 동일 항목)
- **`issue_type=MOBILE_AND_PHYSICAL`의 실물배송(SHIPPING/DELIVERED) 흐름** — 이번 Admin 도메인 설계 범위 밖, 추후 별도 설계 (`docs/api/README.md` Admin 도메인과 동일 항목)
- **`CardDesign` 배정 시점** — 관리자가 신청 접수 직후/사진검토 통과 후/작명 단계 중 언제 배정하는지 (`docs/specs/application/requirements.md` 6절)
- **학번/학과 형식 제약** — 원본 요구사항에 글자수 등 세부 스펙 없음
- **학생증 디자인 시안** — 아직 미도착
- **신청조회(lookup) 전화번호 인증 vs 이메일 인증 조합** — 둘 다 필수인지 하나만이면 되는지 (`docs/specs/application/requirements.md` 3절)
