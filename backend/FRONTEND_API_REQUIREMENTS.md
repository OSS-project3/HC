# 프론트엔드 기준 백엔드 API 요구사항 전수 조사 (Historical Reference)

> **역사 자료 — 현재 상태 판정에 사용하지 않는다.** 이 파일은 2026-08-05 최초 목 화면에서 백엔드 요구사항을 도출한 조사본이라 본문의 “미구현”, “목데이터”, “TODO”는 당시 상태를 뜻한다. 현재 미완료 판정은 `docs/FRONTEND_API_GAPS.md`, 프론트의 실제 연동 방식은 `docs/FRONTEND_API_INTEGRATION_SPEC.md`, 세부 계약은 각 도메인 API 문서를 따른다.
...
| POST | `/api/auth/recovery/id/request` | 이름·전화번호 기반 아이디 찾기 코드 요청 | 없음 |
| POST | `/api/auth/recovery/id/confirm` | 코드 확인 후 마스킹 이메일 안내 | 없음 |
| POST | `/api/auth/recovery/password/request` | 이메일 기반 재설정 코드 요청 | 없음 |
| POST | `/api/auth/recovery/password/confirm` | `requestId + code + newPassword` 검증·저장 | 없음 |
...
#### 회원탈퇴 정책 (2026-08-19 확정 — 소프트 삭제 폐지, 즉시 하드 삭제로 전환)

> 이 절은 원래 "소프트 탈퇴 후 7일 유예기간+자동복구, 7일 후 영구탈퇴(익명화)"였다. 영구 탈퇴 시점에 실제 계정 삭제로 갈지 여부는 "구현 전 신청·후기·감사 로그 등 기존 데이터의 참조 및 보존 의무에 미치는 영향을 먼저 검증해야 한다"고 이 문서에 TODO로 남겨뒀었는데(2026-08-06 시점 작성), 2026-08-19에 그 검증을 실제로 수행하고 정책을 확정했다 — 아래가 그 결과다. **상세 원본은 `docs/collab/user.md`("회원정보·개인정보 보유·탈퇴·파기 정책")가 source of truth이고, `arch.md` §4.1 "탈퇴 정책" 표가 그 요약이다.** 이 절과 원본이 다르면 원본을 따른다.

- **탈퇴는 즉시 확정된다 — 유예기간·자동복구가 없다.** 기존의 "7일 유예기간 안에 로그인하면 자동 복구", `restored: true` 응답 필드는 전부 폐지한다.
- **탈퇴 시 `User` row를 하드 삭제한다**(기존 "익명화 후 row 유지" 방식 폐지) — 별도 배치 스케줄러로 미루지 않고 탈퇴 처리 과정에서 즉시 수행한다. Access Token은 즉시 무효화, `RefreshTokenSession`도 `revoke` 상태로 남기지 않고 함께 하드 삭제한다(토큰 문자열 자체를 남기지 않기 위함).
- **탈퇴 후에는 같은 계정으로 다시 로그인할 수 없다**(row 자체가 없음 — 계정없음과 동일하게 처리).
- **동일 이메일로 재가입은 가능하다.** 단, 완전히 새로운 `User` row로 생성되며 과거 신청·문의·후기 등 `userId` 기준 소유권 데이터는 자동으로 연결되지 않는다.
- **삭제 범위는 `User`와 인증·계정에 종속된 운영 데이터(`RefreshTokenSession`, `ApplicationDailyLimit`)로 한정한다.** `Application`(+`Applicant`/`ApplicationMember`/`Receiver`)·결제/거래 이력은 함께 삭제하지 않고 개인정보처리방침 고지 기준 **상품 수령일로부터 6개월**(전자상거래법상 법정 보존기간이 더 길게 적용될 수 있어 최종 확정 전까지 자동삭제 미구현) 등 각 데이터 자체의 보존기간에 따라 별도로 관리한다. `Inquiry`(문의)는 접수일 기준 6개월(`docs/specs/inquiry/requirements.md` §⑧). `Review`(후기)는 작성자 계정은 삭제되지만 본문은 콘텐츠로서 유지하며, `Board`의 작성 관리자 `created_by_user_id`와 `AdminActivityLog.admin_id`도 이력으로 계속 남긴다(감사기록 무결성 우선, `adminId`를 임의로 변경·삭제하지 않음).
- 개인정보 익명화 스케줄러는 제거한다(하드 삭제라 익명화 대상이 없음). 거래·상담 이력의 보유기간 경과 파기는 이 정책과 완전히 별개의 인프라·스케줄러로 후속 구현한다.
- ⚠️ **미해결**: 현재 전달받은 개인정보처리방침 문안의 "회원가입 정보 보유기간: 상품 수령 후 6개월" 문구가 상품을 신청/수령한 적 없는 회원에게는 기준일이 없다는 문제가 있다 — 이번 즉시 하드 삭제 정책과 어떻게 정리할지는 아직 확정 전(`docs/collab/user.md` §17.1). 법무/개인정보 담당자 확인 필요, 임의로 해석해 구현하지 않는다.

#### 아이디 찾기와 비밀번호 재설정

- 아이디 찾기는 **일반 이메일 계정만** 대상으로 한다. OAuth 전용 계정은 찾기 결과에 포함하거나 `loginMethod`로 노출하지 않는다.
- 아이디 찾기 요청은 이름과 전화번호를 받는다. 정확히 한 개의 일반 계정이 일치할 때만 가입 이메일로 6자리 확인 코드를 보낸다.
- 같은 이름·전화번호로 일반 계정이 여러 개 일치하면 임의 계정을 선택하거나 여러 메일을 보내지 않는다. 일반 성공 응답을 반환하고 고객지원 문의를 안내한다.
- 코드 확인에 성공한 경우에만 로그인 이메일을 마스킹하여 반환한다. 성공해도 자동 로그인하거나 토큰을 발급하지 않는다.
- 비밀번호 재설정 요청은 **이메일만** 받는다. 프론트의 기존 전화번호 입력은 실제 API 연동 시 제거한다.
- OAuth 전용 계정, 존재하지 않는 이메일, 메일 발송 실패는 일반 계정 성공 요청과 동일한 `200 OK` 응답을 반환한다.
- 비밀번호 재설정은 임시 비밀번호를 발급하지 않는다. 요청 응답의 `requestId`와 이메일로 받은 6자리 코드, 사용자가 정한 새 비밀번호를 10분 안에 함께 제출한다.
- 코드 검증 성공 후 새 비밀번호를 BCrypt로 저장하고 코드를 즉시 폐기한다. 해당 사용자의 refresh token과 이미 발급된 access token을 모두 무효화한다.
- 완료 후 자동 로그인하지 않고 로그인 화면으로 이동한다. 비밀번호 변경 알림 메일은 commit 이후 best effort로 발송한다.
...
계정 복구 요청은 계정 존재 여부를 숨기기 위해 다음 공통 정책을 적용한다.

- 요청 응답에는 매번 32-byte URL-safe 난수 `requestId`, `expiresInSeconds=600`, `resendAfterSeconds=60`을 반환한다.
- 코드는 6자리 숫자, 확인 실패 최대 5회, 재발송 60초, 전화번호/이메일당 1시간 5회, IP당 1시간 20회다.
- rate limit과 재전송 제한은 계정 조회 전에 적용하므로 미가입·OAuth·복수 계정도 실제 계정과 동일하게 429가 발생할 수 있다. 429 여부를 계정 존재 판단에 사용하지 않는다.
- Redis 키에는 원문 이메일·전화번호·IP를 넣지 않고 SHA-256 해시를 사용한다.
- 코드 오류·만료·재사용·가짜 요청은 동일한 `INVALID_VERIFICATION_CODE`로 처리한다.
- 아이디 찾기 전화번호 입력은 선택적 선행 `+`, 숫자, 공백, 하이픈을 허용하며 서버가 정규화한다. 프론트가 국가별 표시 형식을 강제로 하나로 변환할 필요는 없다.
- 재설정 후 기존 access/refresh token은 전부 무효화된다. 세션 검증 Redis 장애는 `AUTH_SESSION_VALIDATION_UNAVAILABLE`(503)이므로 프론트는 비밀번호 오류로 표시하지 않고 잠시 후 재시도를 안내한다.
- 상세 요청/응답은 `docs/api/auth.md` API 7·8을 Source of Truth로 사용한다.
...
- 로그인 시도 제한, 재설정 코드의 10분 만료·일회성 소비, 이메일 발송 기록이 필요하다.
