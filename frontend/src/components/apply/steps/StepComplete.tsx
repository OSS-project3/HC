...
export function StepComplete({ draft, applicationNumber, onDone }: StepCompleteProps) {
  ...
  return (
    <div className="step">
      <h2 className="step__heading">{t("신청 완료")}</h2>
      <p className="step__desc">{t(completionMessage[draft.applicantType])}</p>
      <p className="complete__number">
        {t("신청번호")} <strong>{applicationNumber}</strong>
      </p>

      <section className="deposit">
        <h3 className="deposit__title">{t("입금 안내")}</h3>
        <div className="deposit__table">
          <div className="deposit__row">
            <span className="deposit__key">{t("은행")}</span>
            <span className="deposit__val">{t(bankInfo.bankName)}</span>
          </div>
          <div className="deposit__row">
            <span className="deposit__key">{t("계좌번호")}</span>
          ...
          <div className="deposit__row">
            <span className="deposit__key">{t("예금주")}</span>
      ...
      <div className="notice">
        ...
        <ul className="notice__list">
          <li>{t("사전 상담을 통해 확정된 금액을 기준으로 입금해 주시기 바랍니다.")}</li>
          {/* The deposit-deadline line applies to individual applications only. */}
          {!isOrg && (
            <li>{t("신청 후 영업일 3일 이내에 입금해 주시기 바랍니다. 기간 내 입금이 확인되지 않을 경우 신청이 취소될 수 있습니다.")}</li>
          )}
          <li>{t("입금자명과 신청자명이 다를 경우 입금 확인이 지연될 수 있습니다.")}</li>
          <li>{t("입금 확인 후 제작이 진행됩니다.")}</li>
        </ul>
      </div>
      ...
      <div className="step__actions step__actions--end">
        ...
        >
          {t("신청 내역 확인하기")} <ChevronRight width={16} height={16} />
        </Button>
      </div>
    </div>
  );
