...
export function StepType({ draft, update, onNext }: StepTypeProps) {
  ...
  return (
    <div className="step">
      ...
      <h2 className="step__heading">{t("신청 유형 선택")}</h2>
      <p className="step__desc">{t("개인 또는 법인·단체를 선택하면 신청이 시작됩니다.")}</p>
        ...
        {options.map((opt) => {
          ...
          return (
            ...
            >
              ...
              <span className="type-card__label">{t(opt.label)}</span>
      ...
      <div className="notice">
        ...
        <ul className="notice__list">
          <li>{t("신청 유형(개인 / 법인·단체)에 따라 신청 양식이 구분되어 제공됩니다.")}</li>
          <li>{t("원활한 제작 진행을 위해 제작 신청 전 사전 상담을 완료해 주시기 바랍니다.")}</li>
        </ul>
        <label className="check notice__agree">
          ...
          />
          <span>{t("위 안내사항을 확인하였으며, 사전 상담을 완료하였습니다.")}</span>
        </label>
        <label className="check notice__agree">
          <input
            type="checkbox"
          ...
          />
          <span>{t("본 증서는 문화체험용 기념 증서로 제공되며, 법적 신분증으로 사용할 수 없습니다.")}</span>
        </label>
      </div>
