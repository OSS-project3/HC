  ...
  return (
    <section className="auth page-container">
      <div className="auth__card">
        <h1 className="auth__title">약관 동의</h1>
        <p className="auth__lead">서비스 이용을 위해 아래 약관에 동의해 주세요.{user?.name ? ` ${user.name}님, 환영합니다.` : ""}</p>
        ...
        <form className="auth__form" onSubmit={submit}>
          ...
          <Button type="submit" block disabled={!allAgreed || submitting}>{submitting ? "처리 중…" : "동의하고 시작하기"}</Button>
        </form>
      </div>
    </section>
  );
