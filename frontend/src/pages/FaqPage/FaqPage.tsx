  ...
  return (
    <div className="support faq-page">
      <header className="support__hero subpage-hero page-container">
        ...
        <h1 className="support__title subpage-hero__title">{t("자주 묻는 질문")}</h1>
      </header>
      ...
      <section className="support__section page-container">
        ...
        <h2 className="support__heading support__heading--plain">{t("자주 묻는 질문")}</h2>
        <p className="faq__intro">
          {t("자주 문의하시는 내용을 정리했습니다.")}<br />
          {t("원하시는 내용을 찾지 못하신 경우 상담 문의를 이용해 주세요.")}
        </p>

        <div className="faq">
          {status === "loading" && <p className="faq__intro">{t("불러오는 중입니다…")}</p>}
          {status === "error" && <p className="faq__intro">{t("FAQ를 불러오지 못했습니다.")}</p>}
          {status === "ready" && managedFaqs.map((faq, index) => (
            <details key={faq.id} className="faq__item" open={index === 0 ? true : undefined}>
              <summary className="faq__q"><b>Q.</b><span>{t(faq.title)}</span></summary>
              <p className="faq__a"><b>A.</b><span>{t(faq.content)}</span></p>
            </details>
          ))}
        </div>
        ...
        <div className="faq-help">
          ...
          <p>{t("상담 문의")} {companyInfo.phone}　 |　 {t(companyInfo.businessHours)}<br />{t("고객지원에서 다양한 상담 방법을 이용하실 수 있습니다.")}</p>
          <button onClick={() => navigate("/support#contact")}>{t("고객 지원")}</button>
        </div>
      </section>
    </div>
  );
