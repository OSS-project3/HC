    ...
    <div className="company-page" ref={rootRef}>
      ...
      <section className="company-overview page-container reveal">
        <div className="company-overview__lead">
          <h2>{t("이름 하나로,")}<br />{t("한국과 깊이 연결되도록")}</h2>
          <p>{t("여행은 끝나도 이름은 남습니다.")}<br />{t("저희는 한국을 찾은 한 분 한 분께 한글 오행에 기반한 한국 이름을 지어 드리고, 그 이름을 디지털 명예 시민증에 담아 오래도록 간직할 수 있는 기념으로 만들어 드립니다.")}</p>
        </div>
        <div className="company-overview__details">
          <article>
            <h3>{t("정통 명리 성명학")}</h3>
            <p>{t("수십 년간 전통 작명과 명리학을 연구해 온 전문가의 관점으로, 생년월일시와 출생 지역을 해석해 이름에 뜻과 의미를 담습니다.")}</p>
          </article>
          <article>
            <h3>{t("따뜻한 경험")}</h3>
          ...
          <article>
            <h3>{t("디지털 명예 한국인증 · 명예 시민증 · 학생증 · 방문증")}</h3>
            <p>{t("종이가 아닌, 언제 어디서나 꺼내 볼 수 있는 디지털 시민증으로 한국에서의 추억을 안전하게 보관합니다.")}</p>
          </article>
        </div>
      </section>

      <section className="company-promises page-container reveal-group">
        <h2 className="reveal-item">{t("저희가 드리는 약속")}</h2>
        <div className="company-promises__list">
          {promises.map((item) => <article className="reveal-item" key={item.title}><h3>{t(item.title)}</h3><p>{t(item.text)}</p></article>)}
        </div>
      </section>

      <section className="company-process page-container reveal-group">
        <h2 className="reveal-item">{t("이름이 만들어지는 과정")}</h2>
        <p className="reveal-item">{t("복잡한 절차 없이, 네 단계면 나만의 한국 이름과 명예한국인증·명예시민증·학생증·방문증")}</p>
        <div className="company-process__grid">
          {process.map((item, index) => <article className="reveal-item" key={item.title}><span>0{index + 1}</span><h3>{t(item.title)}</h3><p>{t(item.text)}</p></article>)}
        </div>
      </section>

      <section className="company-roadmap page-container reveal-group">
        <div className="company-section-kicker reveal-item">{t("연혁 및 로드맵")}</div>
      ...
      <section className="company-tree reveal">
        <div className="company-tree__inner page-container">
          <img src="/images/logo/tree.png" alt={t("흰 꽃이 핀 이팝나무")} loading="lazy" decoding="async" />
          <div>
            <div className="company-section-kicker">{t("상징수")}</div>
            <h2>{t("이팝나무")}</h2>
            <p>{t("이팝나무는 이밥 나무라고도 합니다.")}</p>
            <p>{t("나무의 꽃잎이 마치 쌀알처럼 생겼고 태조 이성계 덕분에 일반 백성도 쌀밥을 먹게 되면서 이씨(李氏)가 주는 밥이다 하여 이밥 나무라는 이름으로 전해져 내려오고 있습니다.")}</p>
            <p>{t("한글을 창제한 세종대왕이 성씨(姓氏)가 이씨(李氏)이고 한글과 세종에서 한국 이름 지어주는 사업이 이팝나무 꽃잎처럼 수많은 세계인에게 한국 이름을 지어주기를 바라는 뜻이 담겨 있습니다.")}</p>
          </div>
        </div>
      </section>

      <section id="directions" className="company-location page-container anchor-section reveal">
        <div className="company-section-kicker">{t("위치")}</div>
        <h2>{t("찾아오시는 길")}</h2>
        <img
          className="company-location__map"
          src="/images/common/map.jpg"
          alt={t("한글과 세종 찾아오시는 길 약도")}
          loading="lazy"
        />
        <div className="company-location__info">
          <div>
            <p>⌖ {t(companyInfo.address)}</p>
            <p>◷ {t(companyInfo.businessHours)} · {t("주말 및 공휴일 휴무")}</p>
            <p>♧ {companyInfo.phone}</p>
          </div>
          <a href={`https://map.naver.com/p/search/${encodeURIComponent(companyInfo.address)}`} target="_blank" rel="noreferrer">{t("길찾기")}</a>
        </div>
      </section>
    </div>
  );
...
function Timeline({ title, items }: { title: string; items: typeof history }) {
  ...
  return (
    <div className="timeline">
      <h2>{t(title)}</h2>
      <ol>
        {items.map((item) => <li className="reveal-item" key={item.date}><strong>{item.date}</strong><h3>{t(item.title)}</h3><p>{t(item.text)}</p></li>)}
      </ol>
    </div>
  );
}
