...
| 관리자 신청관리(조회·상태전이·작명·카드번호) | ✅ 실 API | ✅ 구현 | **🔴 작명은 절반만 연동** — 조회·상태전이·카드번호는 정상, 인앱 작명·엑셀 작명 반영 둘 다 **성씨(surname) 미전송**이라 `completeNaming()`에서 멤버 전원 거절됨(2026-09-02 재확인, 여전히 미해결, §1.16). 통계(`GET /api/admin/stats`)는 백엔드 자체가 미구현(2026-09-02 재확인) (§1.4) |
