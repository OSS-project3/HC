...
0. **🔴 학생증 schoolName 연동(§1.9-b) — 최우선, 지금 신청이 깨져 있음.** `ApplyPage.tsx` 제출 코드에 한 줄만 추가하면 되는 작은 수정이지만, 안 고치면 학생증 개인·단체 신청이 전부 400으로 막힌다.
1. **일반 이메일 회원가입(§1.1-a)** — 백엔드는 준비됨, 프론트가 인증코드 인라인 입력 UI+실 API 연동을 진행. **로그인·이메일 중복확인·비밀번호 변경(§1.1-b)**도 백엔드는 이미 준비됨, 프론트 연동만 하면 됨. **계정복구(§1.1-c)**는 백엔드 구현이 선행돼야 함(비밀번호 재설정 화면 UX는 이미 결정됨 — 백엔드 API를 그 계약에 맞춰 설계).
