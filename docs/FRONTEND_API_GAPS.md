...
| P1 | 신청 건별 동의 이력 | 신청 전 상담확인·유의사항 체크는 UI 진행 조건이며 생성 DTO/Entity에는 항목·시각·정책 버전이 저장되지 않는다. | 보존 정책 확정 후 신청 생성 계약과 저장 모델 추가 | 정책 및 백엔드 저장 계약 필요 |
...
1. `/me` role 계약과 신청 건별 동의 보존 정책을 백엔드와 확정한다.
