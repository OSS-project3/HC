# 백엔드 남은 작업 조사 스냅샷 — 2026-08-25 (Historical Reference)

> **역사 자료 — 현재 TODO로 사용하지 않는다.** 이후 관리자 통계·공지 검색/페이지네이션·카드 생성·카드 디자인·행사 정적화 등이 구현되어 본문 판정 다수가 낡았다. 현재 프론트에서 확인되는 백엔드 갭은 `FRONTEND_API_GAPS.md` §1을 따른다. 이 문서는 2026-08-25 조사 근거를 보존한다.

이 문서는 2026-08-25 당시 백엔드가 해야 할 것을 모은 조사 스냅샷이다.
