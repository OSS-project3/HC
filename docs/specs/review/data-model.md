...
| application_type | ENUM | NOT NULL | `INDIVIDUAL`, `GROUP` — Application 도메인의 `ApplicationType`과 동일한 enum 재사용(신규 enum 만들지 않음). ⚠️ 실제 `Application` 레코드와 FK로 연결하지 않음 — "이 후기가 개인/단체 신청 경험에 대한 것인지" 자기 신고(self-report) 값으로 설계(아래 §3 참고) |
...
- **`Review → Application` FK**: 후기가 실제 어떤 신청 건에 대한 것인지 시스템이 검증하지 않음(신청 유형/카드 종류는 작성자가 스스로 고르는 값). "진짜 신청자만 후기를 쓸 수 있어야 하는지"는 [TBD — 확인 필요], 필요해지면 `application_id` FK를 추가하는 방향으로 확장 가능.
