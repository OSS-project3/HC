...
| 이름 확정 저장 · 선택이력 집계 | 🟡 **localStorage 시연**(백엔드 미구현) |
| 엑셀 내보내기(이름 포함) | 🟡 **버튼만**(백엔드 미구현) |
| 신청 상태 전이(발급/반려/배송) | 🟡 컨트롤러 미노출 |
...
1. **이름 확정 저장 API** — `POST /api/admin/applications/{id}/members/{mid}/name` (member.name/chineseName 반영 + 선택 카운트 +1). 현재 localStorage.
2. **엑셀 내보내기 API** — `POST /api/admin/applications/export`(xlsx). 백엔드 xlsx 라이터(POI 등) 필요. 현재 버튼만.
3. **만세력 진태양시 보정** — 해외 출생 시차/경도(`birthCities`) 반영(정확도).
4. **신청 상태 전이 관리 API** — 발급/사진반려/배송 등(서비스엔 일부 존재, 엔드포인트 없음).
5. **관리자 승격 정식 경로** — 임시 로그인/시드 제거 후 대체.
6. **후기 전용 admin 목록/모더레이션**(현재 공개 목록 재사용 + 삭제만).
