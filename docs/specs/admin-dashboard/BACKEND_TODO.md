...
| API-5 | 상태 전이(반려/제작/발급/배송/작명완료) | 신청 상태 관리 | — | ✅ **완료**(백엔드 §0.1 + **프론트 연동됨**) |
...
2. (엑셀 왕복 쓸 경우) 프론트에 **결과 엑셀 업로드 UI** 추가 → 기존 `API-0(naming-result)` 연결.
3. 운영 직전 **API-7**(관리자 승격) + 임시 로그인·시드 제거.
4. (선택) 추천 후보·점수화·만세력까지 백엔드로 — DATA-1/4/5 + API-1/4.

> ✅ 이미 완료(백엔드+프론트 연동): **상태 전이(API-5)**, 이름 DB 반영(API-0 엑셀 / API-2 인앱), 선택이력(API-2/2b).
> 확정 이름·선택이력은 **프론트 localStorage를 쓰지 않고 전부 DB에 저장**된다. 상태 전이는 신청 상세 "상태 관리" 바에서 호출.
