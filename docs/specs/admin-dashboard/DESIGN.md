...
- ✅ 현재 상태: 소유자 제공 데이터(`사주 이름 결과.xlsx`) + saju 레포 `names.json`(자원/발음오행)을 병합한
  **실제 이름 700개**를 `frontend/src/data/sajuNames.json`으로 번들하고, `adminNamingMock.ts`가 recommend.py
  점수화 로직(자원오행×2+발음오행×1+상생/상극 보정)을 이식해 오행 결핍 기반으로 8개를 추천한다.
  2026-09-13 정책 확정(`admin-saju.md` "무작위 후보 선택은 사용하지 않습니다") — 이전엔 "매번 무작위
  8개"였으나(정책 위반, `docs/FRONTEND_API_GAPS.md` §1.19 참고), 점수 내림차순+동점 시 사전 순서
  오름차순으로 **결정적**으로 뽑도록 수정. "↻ 다른 이름 추천" 버튼은 재추첨이 아니라 다음 8개(페이지
  넘김)를 보여준다.
