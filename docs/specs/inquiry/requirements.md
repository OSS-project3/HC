...
- **탈퇴 회원의 Inquiry 보존 및 재가입 시 비연결(2026-08-19 확정, 2026-08-19 회원탈퇴 정책 변경에 맞춰 근거 정정)**: `Inquiry.userId`는 `User`와 JPA 연관관계·FK가 없는 순수 `Long` 참조(arch.md §5.1 원칙, `Application.userId`와 동일 방식)이므로 회원 탈퇴가 `Inquiry`에 어떤 cascade도 일으키지 않는다 — 별도 구현 불필요, 설계상 자동 보장. ⚠️ **정정**: 이 근거를 처음 적을 땐 "탈퇴해도 `User` row가 익명화만 되고 삭제되지 않는다"는 당시 소프트 삭제 정책을 전제로 했었다. 이후 회원탈퇴 정책이 **즉시 하드 삭제**로 바뀌었지만(`arch.md` §4.1 "탈퇴 정책"), 이 결론(Inquiry 보존)에는 영향이 없다 — 새 정책의 "삭제 범위 원칙"이 `Inquiry`를 명시적으로 삭제 대상에서 제외하고 §⑧ 보유기간(6개월)을 따르도록 확정했기 때문이다. 즉 `User` row 존재 여부와 무관하게 `Inquiry`는 항상 보존된다. 파생 정책: (1) 탈퇴 후에도 문의는 §⑧ 6개월 보유기간까지 그대로 보존, (2) 관리자는 탈퇴 여부와 무관하게 계속 조회·답변 가능, (3) 답변 알림은 `Inquiry.email`(요청 바디 스냅샷)로 발송되므로 탈퇴 후에도 발송 가능, (4) 탈퇴 회원은 `User` row 자체가 없어 로그인이 불가능하므로 `GET /api/my/inquiries*`로 본인 문의를 더 이상 조회할 수 없다, (5) 재가입 시 신규 `User` row가 새 `id`로 생성되므로(과거 row는 하드 삭제돼 사라짐) 과거 `Inquiry.userId`와 새 계정의 `userId`가 다르다 — 자동 연결 로직을 별도로 만들지 않는다(설계상 자연히 비연결).
