...
관리자가 신청자의 출생정보를 바탕으로 만세력 결과를 확인할 때 적용할 시간대, DST 및 진태양시 계산 책임을 정의합니다.

기존 별도 프로젝트의 `saju/web/src/lib/saju.ts` 계산 결과를 기준으로 결과값을 비교하되, HC에서는 역사적 timezone 및 DST 판정을 Spring 백엔드가 단일하게 담당합니다.
...
별도 `saju/web/src/lib/saju.ts`를 그대로 복사하지 않습니다. 기존 함수에는 브라우저에서 timezone/DST를 판정하는 책임이 포함되어 있어, 그대로 이식하면 Spring과 프론트엔드에서 DST를 중복 계산할 수 있습니다.

이식 시 다음 경계를 적용합니다.

```text
기존 saju.ts
├─ 현지시각 → UTC 변환             Spring으로 이동
├─ 역사적 timezone/DST 판정       Spring으로 이동
├─ 확정 시점의 계산 입력 변환       프론트엔드 유지
├─ 진태양시 보정                   프론트엔드 유지
└─ manseryeok 계산                프론트엔드 유지
```

프론트엔드에는 `utcInstant`와 경도를 입력받아 계산하는 별도 함수를 두고, 해당 함수에서는 timezone offset을 다시 조회하지 않습니다.
...
1. 권한, 상태, 결제, Application `version`, 디자인, 전체 Member 작명 완료 여부, 전 Member 카드번호 존재·형식·유일성을 검증합니다.
...
띠를 생년의 양력 1월 1일 또는 음력 설 기준으로 별도 계산하지 않습니다. 관리자 만세력 계산에서 확정된 연주의 지지를 카드 띠 이미지의 기준으로 사용합니다. 따라서 만세력 엔진의 입춘·절기 계산과 카드 이미지가 서로 다른 결과를 만들지 않습니다.

```text
confirmedPillars.year 존재
→ 연주의 지지로 띠 이미지 결정

연주가 uncertainPillars에 포함
→ 띠 이미지 자동 확정 금지
→ 관리자 확인 필요
```

- `timeAccuracy`가 `EXACT`가 아니더라도 모든 후보의 연주가 같아 `confirmedPillars.year`로 확정되면 자동 매핑할 수 있습니다.
- 연주가 불확실하면 임의의 띠 이미지를 사용하지 않습니다.
- 띠 이미지 결정 결과에는 사용한 만세력 계산 결과 ID 또는 계산 버전을 연결하여 재현 가능하게 합니다.
