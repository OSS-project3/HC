...
- (2026-09-18 추가) 결과 Excel에 `성씨`·`뜻` 헤더가 있으면 각각 선택적으로 함께 반영합니다 — 헤더 자체가 없거나 개별 셀이 비어 있으면 오류가 아니라 그 값만 기존 값 유지(성씨는 `NamingResultExcelParser` §1.16, 뜻은 `ApplicationMember.nameMeaning`에 반영, 긴 풀이 `nameInterpretation`은 이 경로로 반영되지 않음). 현재 saju 쪽 내보내기는 아직 뜻 컬럼을 채우지 않으므로 이 선택 컬럼은 saju가 지원하기 전까지 항상 비어 있는 상태로 스킵됩니다.
