...
| surname_hanja | VARCHAR(10) | NULL | ✅ 2026-09-08 신규 확정: 성씨 한자. **관리자가 입력하지 않고 `surname`으로부터 자동 유도**된다(`ApplicationMember.assignKoreanName` 내부의 고정 매핑 `SURNAME_HANJA`, 원본 사주 데이터 "사주 이름 결과.xlsx"의 성씨별 시트 헤더와 동일한 한자값). 10대 성씨(김金/이李/박朴/최崔/정鄭/조趙/강姜/윤尹/장張/임林)만 매핑되며, 그 외 성씨는 NULL로 남는다. 카드 뒷면 한자 표기는 `CardRenderPreparation.combinedChineseName()`이 `surname_hanja + chinese_name`을 합쳐서 그린다 — `surname_hanja`가 NULL이면 기존처럼 이름 한자만 단독 표시 |
