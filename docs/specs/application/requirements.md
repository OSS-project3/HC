...
- ✅ 2026-08-07 확정: Excel `ID`는 `trim()` 후 String으로 비교하여 `001` 같은 선행 0을 유지한다.
- ✅ 2026-08-07 확정: 사진 확장자는 대소문자를 구분하지 않는다(`001.JPG`, `001.jpg`, `001.Jpg` 모두 허용).
- ✅ 2026-08-07 확정: `__MACOSX`, `.DS_Store`는 무시하고 중간·마지막 빈 행도 무시한다.
- [TBD] ZIP 최대 크기, Excel 최대 행 수, 최대 신청 인원은 현재 제한하지 않으며 TBD로 관리한다(`PENDING_DECISIONS.md`).
