...
- ⚠️ 2026-08-25 재정정(`admin-saju.md` 기준, 이전 "DB에 저장하지 않는다" 방침을 대체): 사진 번호는 ZIP 안에서 사진을 찾기 위한 임시 식별자 역할에 더해, 이제 `ApplicationMember.photo_number`에도 저장한다 — 관리자가 카드번호를 사진 번호 기준으로 일괄 붙여넣을 때 행을 매칭하는 키로 재사용하기 위함이다. `(application_id, photo_number)` 조합은 유일해야 한다.
...
### 5-3. 카드 표기 주소(신규, 2026-08-25)

✅ 확정(`admin-saju.md` 기준): 학생증은 카드에 주소를 표시하지 않는다. 그 외 카드종류(명예한국인증·명예시민증·방문증)는 카드에 주소를 표시하며, 이 주소는 배송용 `Receiver.address`와 별개 값이다.

- 단체 신청: 기존처럼 첨부 엑셀 행의 주소 컬럼을 `ApplicationMember.address`에 저장한다(변경 없음).
- 개인 신청: **신규로 `MemberRequest.address` 입력값을 받아 `ApplicationMember.address`에 저장한다.** 학생증이 아니면 필수, 학생증이면 값이 있으면 거절한다(다른 학생증 전용 필드와 대칭되는 검증).
