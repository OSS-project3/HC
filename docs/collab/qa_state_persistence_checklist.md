...
- [x] **10. 대학교 `SchoolSeeder` — 실제 DB 적재·재실행 통합 테스트 없음 — ✅ 완료(2026-09-20, Claude), 테스트 공백만 있었음(구현은 이미 정책과 일치).** `SchoolSeederTest`는 CSV 파싱 결과만 검사하고, 실제 `SchoolSeeder.run()`을 호출해 DB 적재·재실행 후 중복 없음까지 확인하는 통합 테스트가 없었다. 신규 `SchoolSeederIntegrationTest` 2개 테스트로 확인: 재실행해도 중복 없음(418개 유지), 이미 등록된 학교(전북대학교)는 그대로 두고 누락분만 채움. 구현 자체는 이미 올바르게(존재하는 이름은 보존, 누락분만 추가) 짜여 있었으므로 정책 결정이나 코드 수정 없이 테스트만 추가.
  - ⚠️ 설계 주의사항: `HighSchoolSeederIntegrationTest`(기존, 이 세션 내내 반복 관찰된 무관 플레이키)가 "컨텍스트 기동 시 이미 시드된 상태"에 기대는 방식이라, `schools` 테이블을 공유해서 쓰는 다른 여러 테스트 클래스의 `deleteAll()`에 취약하다는 걸 이번에 직접 확인했다(전체 스위트 실행 시 실제로 2405→2822건으로 변하며 실패 재현). 새 테스트는 같은 함정에 빠지지 않도록 매 테스트가 스스로 초기화하고(단, `HIGH_SCHOOL` 행은 건드리지 않도록 `UNIVERSITY` 타입만 골라 지움 — `deleteAll()`을 그대로 썼다면 `HighSchoolSeederIntegrationTest`를 새로운 플레이키로 만들었을 것), 개수 검증도 `UNIVERSITY`로만 필터링해 전체 스위트 실행 순서와 무관하게 결정적으로 통과하도록 설계했다. 실제로 전체 회귀에서 `HighSchoolSeederIntegrationTest`는 여전히(무관하게) 플레이키했지만 신규 테스트는 영향받지 않음을 확인.
  - 전체 회귀 961개 중 동일한 무관 플레이키(`HighSchoolSeederIntegrationTest`) 1개 제외 통과.
