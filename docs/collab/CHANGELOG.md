...
- 파일: `CardImageCompositor.java` — `composeBack()`(다른 3종 공용)과 `composeStudentBack()` 양쪽의 타이틀 `drawBackText` 호출에서 `batangBold` → `dotumBold`로 변경. 이름·한자·영문명 등 나머지 필드 폰트는 그대로 유지(타이틀만 범위).
- 사유: 사용자 제보(실제 렌더링 결과가 디자인 시안과 다름) — 타이틀만 고쳐 나머지 필드의 기존 타이포그래피(이름=batangBold 강조, 본문=dotumMedium)는 건드리지 않았다.
- 테스트: 백엔드 전체 회귀 1024개 이상 전부 GREEN(`BUILD SUCCESSFUL`, 실패 0 — 직전 1회는 `hc-test-redis`가 Docker Desktop 재시작으로 죽어있어 208건 실패했었는데, 이 세션에서도 반복 확인된 무관한 인프라 이슈였고 컨테이너 재시작 후 재실행해 전부 통과 확인). dev 컨테이너를 재빌드해 실제 카드를 재생성 — 학생증(APP-2026-000012)과 명예한국인증(APP-2026-000001) 양쪽에서 "한국이름풀이" 타이틀이 굵은 고딕체로 바뀐 것을 실제 PNG 다운로드로 직접 확인.
- 관련: 사용자 제보 기반 즉시 수정(TODO.md 항목 없음).

---
## 2026-09-25 — Codex — `main` (로그인 잠금 시 전용 안내 문구 — 프론트)

- 변경: 백엔드는 이미 로그인 반복 실패 시 `ACCOUNT_LOCKED`(429)를 던지고 있었지만, 프론트는 이를 구분하지 않고 일반 401/400/404와 동일하게 "아이디/비밀번호가 틀리거나 존재하지 않는 계정입니다"로 뭉뚱그려 보여주고 있었다. `err.code === "ACCOUNT_LOCKED"` 분기를 추가해 "로그인 시도가 너무 많아 계정이 15분 동안 잠겼습니다..." 전용 안내와 비밀번호 찾기 유도 문구를 보여주도록 수정.
- 파일: `LoginPage.tsx`(`ACCOUNT_LOCKED` 분기 추가), `features/i18n/translations/auth.ts`(영문 번역 추가) — 커밋 `6261caa`(Codex가 워킹 트리에 직접 작성, Claude가 커밋·push).
- 사유: 계정 잠금 상황에서 사용자가 원인을 알 수 없이 "비밀번호가 틀렸다"는 오해를 하게 되는 UX 결함 수정.
...
- **구현 중 자체적으로 발견해 사용자에게 확인받은 설계 위험 1건**: 마지막 멤버 이름 확정 시 곧바로 `PRODUCTION_READY`로 자동 잠기면, `ApplicationStatus`에 `PRODUCTION_READY→NAME_EDITING` 역방향 전이가 없어서 그 이후로는 어떤 멤버의 이름도 다시 고칠 방법이 없어지는 회귀였다(자동 전이 전에는 관리자가 "작명완료 처리"를 늦게 눌러 그 사이 계속 수정할 수 있었음). `AskUserQuestion`으로 두 가지 안(그대로 감수 vs. `PRODUCTION_READY`+카드 미생성 멤버는 재확정 허용)을 제시해 사용자가 후자를 선택("PRODUCTION_READY에서도 이름 재확정 허용(추천)") — `assignMemberName()`의 게이트를 `requireMemberNameEditable()`로 분리해 `NAME_EDITING`은 항상 허용, `PRODUCTION_READY`는 해당 멤버 카드가 아직 없을 때만 허용, 그 외는 거절하도록 구현했다. `applyNamingResult()`(엑셀 일괄 반영)의 게이트는 의도적으로 건드리지 않아 여전히 `NAME_EDITING`만 허용한다.
- 파일: (백엔드 수정) `ApplicationService.java`(`requireCardGenerationComplete`를 에러리스트 반환 `cardGenerationErrors`+throw wrapper로 분리, 신규 `tryAutoStartProducing`/`autoCompleteNamingIfReady`/`requireMemberNameEditable`, `assignMemberName`/`applyNamingResult`에서 저장 후 자동완료 체크 호출), `CardGenerationService.java`(카드 저장 커밋 후 `tryAutoStartProducing` 호출, 실패는 로깅만 하고 응답은 그대로 성공) / (백엔드 테스트) `ApplicationServiceNameAssignTest.java`(기존 "NAME_EDITING 아니면 거절" 테스트를 새 경계에 맞게 5건으로 재작성 — PRODUCTION_READY+카드미생성 허용/카드생성됨 거절/무관상태 거절/마지막멤버 자동완료/단체 부분완료 시 미전이), `CardGenerationServiceTest.java`(기존 "상태 안 바뀜" 단언을 "PRODUCING으로 자동 전이됨" + 감사로그 단언으로 갱신, 단체 신청 부분완료/전원완료 경계 신규 테스트 2건 추가) — 아직 커밋 전.
- 사유: 사용자가 명시적으로 "우선 백엔드부터 시작해주세요"라고 지시해 프론트(드롭다운 제거, 버튼 재배치, 읽기전용 상태표시기)는 이번 커밋 범위에 포함하지 않음 — 백엔드 완료·전체 회귀 확인 후 별도 확인 받고 착수할 것.
- 테스트: 구현 중 기존 테스트 픽스처의 잠재 갭 2건을 발견해 같이 고쳤다(제품 코드 버그 아님) — ①`CardGenerationServiceTest` 기존 픽스처가 `ReflectionTestUtils`로 `status`만 `PRODUCTION_READY`로 찍고 `paymentStatus`는 안 세팅해서 `Application.startProducing()`의 기존 결제확인 검증에 걸림(`paymentStatus=CONFIRMED` 추가로 해결), ②신규 단체 테스트 픽스처가 `logoFileId`/`sealFileId`를 `null`로 둬서 `CardRenderPreparation.validateIssuerAssets()`의 기존 "단체는 로고·직인 필수" 검증에 걸림(실제 `UploadFile` 행을 만들어 ID 전달로 해결). 두 갭 모두 diagnostic 테스트(이후 삭제)로 root cause를 격리해서 확인했다. 타겟 테스트(`ApplicationServiceNameAssignTest`+`CardGenerationServiceTest`) 전부 GREEN, 전체 회귀 1024개 중 1023개 통과(무관 플레이키 `HighSchoolSeederIntegrationTest` 1건, 이 세션에서 반복 확인된 기존 이슈 — 단독 실행 시 통과).
- 관련: `docs/collab/TODO.md` "관리자 상태 전이 — 드롭다운 제거, 업무 버튼 + 자동 전이" 절 (Backend 항목 전부 완료, Frontend는 대기)

---
## 2026-09-24 — Claude — `main` (관리자 상태 전이 — 드롭다운 제거, 업무 버튼 + 자동 전이 — 프론트엔드)

- 변경: 백엔드 완료(바로 아래 항목) 후 사용자가 "정해야할 정책없이 바로 코드 작성가능하면 작성해줘. 모르는 부분이 있으면 구현하지말고 물어봐줘"로 프론트 착수를 승인 → 정책 결정이 필요 없는 항목은 바로 구현하고, 유일하게 남아있던 미정 사항(자동 전이 시 관리자에게 별도 알림을 줄지)만 `AskUserQuestion`으로 확인받았다. `ApplicationDetail.tsx`의 범용 "상태 변경" `<select>`(8개 액션을 현재 상태에 따라 필터링해 드롭다운 하나로 보여주던 방식)를 제거하고, 남는 6개 업무 액션(입금확인/검토시작/작명승인/사진반려/카드발급완료/배송발송)을 각각 독립 버튼으로 바꿨다. "작명 완료 처리"/"제작 시작"은 버튼 목록에서 아예 제거했다 — 백엔드가 이미 자동 처리하므로 평소 업무 흐름에는 노출하지 않는다(수동 엔드포인트 자체는 백엔드에 여전히 남아있어 필요 시 API로 복구 가능). 전체 파이프라인(`SUBMITTED→REVIEWING→NAME_EDITING→PRODUCTION_READY→PRODUCING→COMPLETED`)을 보여주는 토글형 읽기 전용 상태 표시기를 신설했다 — "전체 진행 단계 보기" 버튼을 누르면 주 경로 6단계(완료/현재/예정 색상 구분)와 이탈 경로 2개(사진반려·취소, 현재 상태면 "(현재)" 표시)가 펼쳐진다. 백엔드 `ApplicationStatus.canTransitionTo`의 실제 전이 그래프(주 경로는 선형, `PHOTO_REJECTED`는 `REVIEWING`에서 갈렸다 되돌아옴, `CANCELLED`는 여러 단계에서 도달 가능한 종단)를 그대로 반영했다.
- 자동 전이 알림 정책(`AskUserQuestion`으로 확인): "기존 저장 토스트만 유지(추가 없음)"으로 확정 — 마지막 멤버 이름 확정/카드 생성 시 이미 뜨는 저장 성공 토스트("이름을 확정했습니다" 등) 외에 별도 "자동으로 OOO 상태로 전이됐습니다" 같은 토스트는 추가하지 않는다. 상태 배지·파이프라인 표시기는 기존에도 매 저장 후 호출되던 `onSaved`→`reloadDetail()`이 그대로 최신 상태를 반영하므로 이 결정에 따른 추가 구현은 필요 없었다.
- 파일: (프론트 수정) `ApplicationDetail.tsx`(드롭다운 제거 → 버튼 목록, `statusActions`에서 작명완료/제작시작 제거, `PIPELINE_MAIN`/`PIPELINE_BRANCHES` 상수 + 토글 상태 + 파이프라인 뷰 신규), `AdminPage.css`(`.admin-naming__status-actions`/`.admin-naming__pipeline*` 신규, 더 이상 안 쓰는 `.admin-naming__status-select` 규칙 제거) / (문서) `docs/collab/TODO.md`(Frontend 체크리스트 완료 표시, 진행 보드 ✅로 갱신).
- 사유: 사용자가 명시적으로 "정해야할 정책없이 바로 코드 작성가능하면 작성해줘"로 프론트 착수를 승인. 검토시작/작명승인/배송발송을 계속 독립 버튼으로 유지할지는 이전 세션에서 사용자가 명시적으로 답하지 않은 채 넘어간 항목이라, TODO.md에 이미 기록해둔 "별다른 이견 없어 그대로 유지" 가정을 그대로 따랐다(재질문하지 않음, 프론트 작업이라 이번에 실제로 적용됨).
- 테스트: `tsc --noEmit` strict, `npm run build` 둘 다 통과. dev 컨테이너를 백엔드·프론트 모두 재빌드해 Playwright로 실제 확인 — 기존 `<select aria-label="상태 변경">`이 DOM에서 완전히 사라졌음, "전체 진행 단계" 토글이 정상 렌더링·클릭 시 펼쳐짐(주 경로 6단계 텍스트가 정확한 순서로 표시, 현재 단계 "접수"가 강조, 이탈 경로 줄에 "사진반려 · 취소" 표시), SUBMITTED+결제대기 상태에서 "결제 확인" 버튼 1개만 노출되는 것까지 스크린샷으로 확인.
- 관련: `docs/collab/TODO.md` "관리자 상태 전이 — 드롭다운 제거, 업무 버튼 + 자동 전이" 절 — 이 기능은 이제 백엔드+프론트 모두 완료.
...
- 변경: 백엔드 완료(아래 항목) 후 사용자 승인("정해야할 정책이 완전히 확정됐다면 그대로 진행")을 받아 프론트를 구현했다. `ApplicationDetail`의 십이간지 native `<select>`를 `ZodiacDesignSelector`로, `CardProductionPanel`의 카드 디자인 native `<select>`를 `CardDesignSelector`로 교체 — 둘 다 옵션에 hover/focus/터치하면 그 항목의 원본 이미지를 조회 전용으로 미리 보여주는 custom listbox다(Codex 정책: PC hover·focus는 조회만, 클릭·Enter/Space·모바일 touch만 로컬 선택값 변경, DB 저장은 기존 저장·카드생성 흐름에서만). **다만 이번 작업은 구현보다 검증에서 값어치가 나왔다** — dev 컨테이너를 매 수정마다 재빌드하고 실제 Playwright로 상호작용을 검증하는 과정에서 코드 리뷰만으로는 못 잡았을 실제 버그 3건을 발견해 고쳤다:
  1. **이미지가 영원히 안 뜨는 자기 취소 버그**: 항목별 미리보기 fetch를 담당하는 `useEffect`가 `cache`(state)를 의존성에 넣고 있었는데, 그 안의 `setCache(..., "loading")`이 그 즉시 같은 effect를 재실행시켜 cleanup의 `cancelled=true`가 방금 시작한 요청 자신을 취소해버렸다 — 네트워크 호출은 200으로 성공하는데 응답이 절대 state에 반영되지 않아 이미지가 계속 스켈레톤인 채 멈춰 있었다. `requested` ref로 "이미 요청함" 가드를 분리해 수정.
  2. **패널이 통째로 안 보이는 CSS 클리핑**: 이 셀렉터들은 관리자 신청 목록 테이블의 펼친 상세 행 안에 있고, 그 테이블은 가로 스크롤용 `.admin__table-wrap { overflow-x: auto; }`로 감싸여 있다. CSS Overflow 스펙상 overflow-x/overflow-y 중 하나가 `visible`이고 다른 하나가 아니면 두 축 다 `visible`일 수 없어 `visible` 쪽이 강제로 `auto`로 바뀐다(`overflow-y: visible`을 명시해도 소용없음 — 스펙상 같은 취급) — 그래서 일반 `position:absolute` 패널은 이 래퍼의 콘텐츠 기준 높이를 넘는 순간 그냥 잘려서 안 보였다. 기존 native `<select>`는 브라우저 자체 팝업 레이어라 이 문제를 겪지 않았던 것. `document.body`로 포탈 렌더링(`position:fixed`, JS로 트리거 좌표 계산)해 구조적으로 회피 — 이 상호작용에서 두 컴포넌트가 완전히 같은 코드가 필요해져 `usePreviewPopover` 공용 hook으로 추출했다(중복이면 한쪽만 고치고 잊어버릴 위험이 실제로 이 세션에서 증명됨).
  3. **열자마자 닫히는 버그**: 포탈 패널을 트리거에 붙여두려고 연 상태에서 scroll/resize 리스너로 재계산하게 해뒀는데, 처음엔 "닫기"로 구현했었다. scroll은 버블링하지 않지만 `capture:true`는 옵션 목록 자체의 내부 스크롤(`overflow-y:auto`, 방향키·자동포커스의 `scrollIntoView`)까지 잡아채 열자마자 다시 닫아버렸고, 모바일에서는 주소창이 접히고 펴지며 계속 쏘는 `resize`도 같은 문제를 일으켰다. "닫기" 대신 "재계산"으로 바꿔 해결. 이 과정에서 패널 높이가 뷰포트(특히 모바일 짧은 화면)보다 커질 수 있고, `position:fixed`라 페이지 스크롤로도 넘치는 부분에 닿을 수 없다는 것도 추가로 발견해 트리거 아래 남은 세로 공간으로 `max-height`를 제한하고 패널 자체를 내부 스크롤시켰다.
  키보드 방향키 이동도 처음엔 빠져 있었다(Codex 체크리스트가 명시했는데 놓침) — 트리거를 열어도 DOM 포커스가 트리거 버튼에 남아있어 ArrowDown이 목록에 닿지 못했던 것으로, 패널이 마운트되면 선택된(또는 첫) 옵션으로 자동 포커스하도록 추가해 해결(Playwright로 wraparound까지 확인).
- 파일: (프론트 신규) `ZodiacDesignSelector.tsx`, `CardDesignSelector.tsx`, `usePreviewPopover.ts` / (프론트 수정) `services/api.ts`(응답 타입 3개 + API 함수 2개), `ApplicationDetail.tsx`(십이간지 select 교체), `CardProductionPanel.tsx`(카드 디자인 select 교체), `AdminPage.css`(팝업·그리드·체크무늬·스켈레톤 스타일) — 커밋 `a3d588f`(초기 구현) → `24ce146`(키보드 nav) → `d3731be`(scroll/resize 재계산 + 모바일 높이) / (문서) `docs/collab/TODO.md`(Frontend 체크리스트·완료검증 갱신)
- 사유: 사용자가 "정해야할 정책이 완전히 확정됐다면 그대로 진행"으로 승인 → `docs/collab/TODO.md`의 이미 확정된 정책·API 계약·체크리스트 그대로 구현. 검증 방식은 이 세션에서 반복 확인된 원칙("코드가 옳아 보이는 것"과 "실제로 동작하는 것"은 다르다)을 그대로 적용 — 매 수정마다 dev 컨테이너를 재빌드하고 실제 브라우저로 확인했다.
- 테스트: `tsc --noEmit`/`npm run build` 매 수정마다 통과. Playwright로 실제 확인: (1) 십이간지 5세트 hover → 세트당 정확히 1회 요청, 재hover 시 재요청 없음, 세트 4/5 체크무늬 배경 위 실제 이미지 렌더(스크린샷), (2) 카드 디자인 hover → 앞·뒷면 이미지 정상 렌더(naturalWidth로 실제 디코딩 확인), (3) 클릭/Enter로 로컬 선택 후 팝업 닫힘 + DB 저장 API 미호출, (4) `locked`(카드 이미 생성됨) 상태에서 클릭은 no-op이지만 다른 디자인 계속 조회 가능함을 실제 확정된 신청으로 확인, (5) Escape/바깥클릭으로 닫힘, 패널 내부 클릭으로는 안 닫힘, (6) 방향키 Up/Down/Home/End + wraparound + Enter 선택, (7) 네트워크 강제 실패 → 재시도 버튼 → 성공까지. **모바일 touch만 자동 검증 실패** — Playwright `.tap()`과 raw `touchscreen.tap()` 둘 다, 패널이 안정적으로 열려있는 상태에서도 이벤트 자체가 버튼에 안 닿는 원인 불명의 한계에 부딪혔다(여러 방식으로 반복 확인, 코드는 표준 `<button onClick>`이라 터치→클릭 변환을 막는 로직이 없음 — headless Chromium/Playwright 모바일 에뮬레이션과 `position:fixed` 포탈의 상호작용 한계로 판단). 실기기 수동 QA 필요.
- 관련: `docs/collab/TODO.md` "십이간지·카드 디자인 선택 이미지 미리보기" 절

---
## 2026-09-23 — Claude — `main` (십이간지·카드 디자인 선택 이미지 미리보기 — 백엔드, Codex 정책 구현)

- 변경: Codex가 `docs/collab/TODO.md`에 확정 정책·체크리스트로 남긴 "십이간지·카드 디자인 선택 이미지 미리보기"(2026-09-22 확정)의 **백엔드**를 사용자 지시("구현 진행하되 백엔드부터 구현해줘")대로 구현했다. 읽기 전용 GET API 2개 — `GET /api/admin/zodiac-designs/{designSet}/preview`(세트 1~5, 대표 4종 쥐/호랑이/용/돼지를 `RAT`/`TIGER`/`DRAGON`/`PIG` 고정 순서로 반환), `GET /api/admin/card-designs/{cardDesignId}/preview`(카드 디자인 앞·뒷면 원본을 base64로 반환, 일반 카드는 classpath/학생증은 S3). 신규 `CardAssetPreviewService`는 `AdminAuthorizationService`/`CardDesignRepository`/`CardTypeRepository`/`UploadFileRepository`/`StorageService`에만 의존하며 기존 `CardImageCompositor`·`CardDesignService`는 전혀 수정하지 않았다. `CardImageCompositor`를 다시 읽어보니 Codex 스펙 문구("앞면.png와 뒷면.png")보다 실제 로직이 더 복잡했다 — 후보 파일명을 순서대로 탐색(`앞면.png`→`대지 1.png`, `뒷면.png`→`대지 1 사본.png`)해야 하고, VISITOR 디자인 1은 실물 리소스 확인 결과 `대지 1.png` 한 장뿐이라 앞/뒤 구분이 불가능해 `isUnverifiedDesign` 가드를 그대로 복제해 `CARD_DESIGN_NOT_FOUND`로 거절하도록 했다. 렌더링(합성)이 아니라 원본 바이트만 base64로 돌려주면 되므로 `CardImageCompositor.loadImage()`의 AWT 팔레트 PNG 워크어라운드는 불필요해 단순한 `ClassPathResource`/`StorageService.download()` 읽기로 구현했다.
- 파일: (백엔드 신규) `ZodiacAnimalPreview.java`, `ZodiacDesignPreviewResponse.java`, `CardDesignTemplatePreviewResponse.java`, `CardAssetPreviewService.java`, `ZodiacDesignController.java`, `CardAssetPreviewServiceTest.java`, `CardDesignControllerTest.java`, `ZodiacDesignControllerTest.java` / (백엔드 수정) `CardDesignController.java`(preview GET 1개 추가, 기존 list API 불변) — 커밋 `3336b6f` / (문서) `docs/api/card-design.md`(API 3 신규 절), `docs/specs/application/admin-saju.md`("십이간지·카드 디자인 선택 이미지 미리보기" 절 신규), `docs/collab/TODO.md`(Backend 체크리스트 13개 전부 완료 표시, 진행 보드 갱신)
- 사유: 사용자가 세션 초반 "십이간지 캐릭터나 카드 디자인 셀렉시 토글 선택할 때 옆에 포인터를 갖다대면... 이미지가 떴으면 좋겠다"고 요청한 것을 Codex가 별도로 정책·API 계약·체크리스트까지 확정해 `TODO.md`에 남겨둔 걸 발견 → 사용자에게 보고 후 "구현 진행하되 백엔드부터 구현해줘" 지시를 받아 착수.
- 테스트: 신규 3개 테스트 클래스(총 20개 테스트) 전부 GREEN. 전체 회귀 1001개 중 1000개 통과 — 실패 1건(`HighSchoolSeederIntegrationTest.reseedingTheSameCsvDoesNotDuplicateRows`)은 단독 실행 시 통과함을 재확인해 전체 스위트 실행 순서에 따른 기존 플레이키(이번 변경과 무관, 카드/십이간지 도메인과 접점 없음)로 판단. 로그는 `build/logs/zodiac-card-preview-focused.log`, `build/logs/full-regression-after-preview-backend.log`, `build/logs/highschool-seeder-isolated.log`.
- 관련: `docs/collab/TODO.md` "십이간지·카드 디자인 선택 이미지 미리보기 (2026-09-22 확정)" 절. 프론트엔드(ZodiacDesignSelector/CardDesignSelector 등)는 사용자 지시에 따라 이번 작업 범위에서 제외 — 별도 확인 후 착수 예정.
...
- 파일: `docs/collab/TODO.md`.
- 사유: 사용자가 "isDefault가 실제로 active와 별개 의미가 있는지, 그리고 STUDENT엔 이 필드가 필요한지" 검토를 요청 — 추측 대신 실제 코드를 grep해서 확인.
- 관련: TODO 4-D

---

## 2026-08-31 — Claude — `main` (4-D 착수 전 검토 — 미정 지점 6건 정리 + 뒷면 placeholder 실물 선정)

- 변경: 4-D(학생증 템플릿 업로드 API) 코드를 짜기 전에 문서상 안 정해진 지점 6개(CardDesign 학교 불변조건을 어디서 강제할지, designNumber 동시 채번 경쟁, 자동생성 CardDesign의 name/isDefault 값, POST 응답 바디 형태, MIME/해상도 검증 실패 에러코드, 뒷면 타이틀 텍스트 색상 고정으로 인한 저대비 문제)를 정리해 TODO.md에 남겼다. 마지막 항목은 실제로 뒷면 placeholder 후보 5개(다른 학교 앞면 시안 재활용)를 전부 렌더링해보다가 발견한 실측 결함 — 진한 색 상단 밴드 디자인 위에서 검정 타이틀 텍스트 가독성이 떨어짐. 사용자 요청대로 뒷면 실물 에셋이 없는 동안 임시로 쓸 이미지도 골랐다(세로 `아트보드 8 사본 10.png`, 가로 `아트보드 8 사본 15.png`). 완료 조건에 사용자가 지정한 통합 테스트 항목(업로드→실제 렌더링까지 글자 깨짐 없이 확인)도 추가했다.
- 파일: `docs/collab/TODO.md`.
- 사유: 4-D는 다음 착수 대상으로 확정됐고, 코드 짜다가 즉흥적으로 정하기보다 미리 정리해두는 게 낫다고 판단.
- 관련: TODO 4-D

---

## 2026-08-31 — Claude — `main` (TODO.md 0-A~3-A 체크리스트를 실제 4-B/4-C 진행 상태와 동기화)

- 변경: `docs/collab/TODO.md`의 "단계별 실행 체크리스트"(2026-08-27 작성, 0-A~3-A)가 그 이후 완료된 4-B/4-C 구현과 동기화되지 않고 있던 걸 사용자 질문으로 발견 — 2-A/2-B 항목별로 실제 코드·테스트 상태를 재확인해 완료 표시와 근거(테스트명·커밋)를 채워 넣었다. 임의로 다 체크하지 않고, 실제로 자동 테스트가 없는 항목(CardPreviewService/AdminApplicationController의 STUDENT 케이스, CardDesign 학교 불변조건 강제, DB→CardMemberData 매핑 검증)은 미체크로 남기고 왜 아직 안 됐는지 적었다.
- 파일: `docs/collab/TODO.md`.
- 사유: 문서 신뢰성 — 다음 작업자가 27일자 체크리스트만 보고 이미 끝난 작업을 다시 하거나, 반대로 실제로 비어있는 자동 테스트 공백(Preview/Controller 레벨 STUDENT 검증 없음)을 놓치지 않도록.
- 관련: TODO 4-B/4-C, 2-A/2-B
...
- 파일: `backend/honor-citizen/src/main/resources/card-templates/HONOR_CITIZEN/2/직인.png`(신규), `TODO.md`.
- 검증: `HONOR_CITIZEN/1`·`HONOR_CITIZEN/2` 둘 다 재렌더링해 직인이 캔버스 밖으로 잘리지 않고 정상 표시됨을 육안 확인.
