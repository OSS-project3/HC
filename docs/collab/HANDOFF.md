...
- **일반 이메일 인증·로그인 전체 흐름 완료**: 회원가입 4단계(MAIL-1→SIGNUP-1→SIGNUP-2→AUTH-4)에 이어 로그인까지 끝났다.
  ...
  - 전체 스위트 427개 중 아래 "발견된 기존 결함" 1건만 실패(회귀 없음). 상세는 `TODO.md` 백엔드 API 연동 체크리스트 절, `CHANGELOG.md` 2026-08-19 항목들 참고.
...
- **문서 갱신**: `docs/FRONTEND_API_INTEGRATION_SPEC.md`/`docs/FRONTEND_API_GAPS.md`의 "로그인 BLOCKED" 표기를 AUTH-5 완료로 갱신(위 참고).
- **AUTH-3**: `POST /api/auth/email/check` — 다른 어떤 단위와도 의존관계 없어 언제 진행해도 됨.
- **AUTH-6**: `PATCH /api/users/me/password` — AUTH-5까지 끝나 선행조건 충족, 다음 순서로 자연스러움.
...
- MAIL-1/SIGNUP-1/SIGNUP-2/AUTH-4/RATE-1/AUTH-5 전부 신규 테스트(집중) 실행 후 전체 스위트(394/400/406/411/416/427개, 회귀 테스트)를 돌려 확인했다 — 실패 1건(`UserApplicationFlowTest`)은 위에 기록한 대로 기존 결함, 여섯 단위 모두 동일건으로 회귀 아님을 재확인.
