...
- 작성자: Claude
- 브랜치: main
- 커밋·push: `86203af`까지 push 완료(사용자 요청으로 그 시점에 push). 그 이후(관리자 카드 개별 다운로드, 관리자 상태 전이 자동화 백엔드) 신규 변경은 아직 push 요청 없었음.

## 현재 워킹 트리

**미커밋 상태** — 아래 "진행중 — 관리자 상태 전이 자동화(백엔드)" 절의 소스·테스트·`docs/collab/TODO.md`/`CHANGELOG.md` 변경이 아직 커밋되지 않았다. 전체 회귀 테스트는 GREEN 확인 완료(무관 플레이키 1건 제외), 다음 세션은 커밋부터 시작하면 된다.

## 진행중 — 관리자 상태 전이 — 드롭다운 제거, 업무 버튼 + 자동 전이 (2026-09-24 정책 확정, 백엔드 완료·커밋 전)
...
- 백엔드 구현·테스트 전부 완료·GREEN(타겟 테스트 + 전체 회귀 1024개 중 1023 통과, 나머지 1건은 무관 플레이키). 상세는 `docs/collab/CHANGELOG.md` 2026-09-24 "관리자 상태 전이... 백엔드" 항목, 체크리스트는 `docs/collab/TODO.md` 동일 절 참고.
- **다음에 할 일**: (1) 위 변경을 커밋(소스+테스트 한 커밋, 문서 갱신은 같은 커밋 또는 뒤따르는 커밋으로 — 이 세션의 관례대로), (2) 사용자가 명시적으로 "백엔드부터"라고 지시했으므로 프론트(드롭다운 제거, 남는 업무 버튼 재배치, 읽기전용 상태-파이프라인 표시기 신설)는 별도 확인 없이 착수하지 말 것.
- **미확답으로 남은 사소한 결정**: 검토시작/작명승인/배송발송을 프론트에서 계속 독립 버튼으로 유지할지 사용자가 명시적으로 답하지 않았음(다른 질문에 답하며 자연히 넘어감) — 이번 백엔드 작업엔 영향 없음(이 세 액션의 서비스 로직은 안 건드림), 프론트 작업 착수 시 한 번 더 확인할 것.
