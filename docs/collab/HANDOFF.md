...
- **✅ 회원탈퇴 정책 변경 확정(2026-08-19) — 문서만, 코드 미착수**: 사용자가 소프트 삭제(7일 유예+익명화)를 폐지하고 즉시 하드 삭제로 바꾸자고 제안 → 먼저 스코프 분석(코드베이스 전체에서 `User.id`를 참조하는 7개 테이블 전수 확인, `arch.md` §5.1의 FK-없는-Long-참조 원칙 덕분에 하드 삭제해도 DB/화면 레벨 문제가 없음을 확인) → 사용자가 최종 정책표로 확정.
  ...
  - **✅ "회원가입 정보 보유기간(상품 수령 후 6개월)" 정합성 — 2026-08-19 해소**: 6개월은 탈퇴하지 않고 계속 활동하는 회원의 기본 보유기간(만료 시 파기 대상)이고, 회원탈퇴 시에는 그 기간을 기다리지 않고 지체 없이 파기한다 — 탈퇴/보유기간만료 중 먼저 오는 사유로 파기(`docs/collab/user.md` §1.1·§17.1). **여전히 남은 미해결**: 비밀번호 제3자 제공 문구 오류 의심(§17.2), 제3자 제공/처리위탁 조항 혼재(§17.3) — 법무 확인 필요, 임의 해석 금지.
