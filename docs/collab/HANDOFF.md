...
- **이메일 회원가입 인증 작업 진행 중 (MAIL-1 → SIGNUP-1 → SIGNUP-2 → AUTH-4 중 2/4 완료)** — 사용자가 지시한 4단계 작업.
  - **MAIL-1 완료·커밋됨**(`a79c08c`): `spring-boot-starter-mail`+`EmailSender`/`SmtpEmailSender` 인프라. 상세는 `TODO.md` MAIL-1 항목 참고.
  - **SIGNUP-1 완료, 이번 커밋으로 반영**: `POST /api/auth/signup/email-verification/request` — 정책 9단계(정규화→형식검증→중복확인→재전송/횟수제한→코드생성→HMAC저장→TTL10분→SMTP발송→응답) 그대로 구현. 신규 테스트 6개 전부 **실제 로컬 Redis**(Docker `honor-citizen-redis-test`, 호스트 포트 6400)로 통과. 상세는 `TODO.md` SIGNUP-1 항목, `CHANGELOG.md` 2026-08-19 항목 참고.
  - **다음 단위: SIGNUP-2**(코드 확인 API, `POST /api/auth/signup/email-verification/confirm`) — 아직 미착수.
...
- **SIGNUP-2 착수**: `POST /api/auth/signup/email-verification/confirm` — 코드 검증(최대 5회, 실패 시 challenge 폐기), 성공 시 32바이트 URL-safe 가입 토큰 발급(Redis엔 SHA-256 해시만 저장, normalizedEmail 연결, TTL 30분), 코드 확인+실패횟수 변경은 Redis Lua로 원자 처리. 상세 정책은 `TODO.md` SIGNUP-2 항목 참고.
- **이어서 AUTH-4**: `POST /api/auth/signup` — signupToken+email 필수, 토큰 해시로 Redis 조회 후 이메일 일치 검증, DB 중복 재조회, BCrypt 해시, `User.createLocalUser` 저장, **DB commit 성공 후에만** 가입 토큰 삭제, 인증 쿠키 발급. 토큰-이메일 불일치는 이메일 존재 여부를 노출하지 않는 동일 오류로 처리.
...
- SIGNUP-1 작업은 신규 테스트 6개(집중) 실행 후 전체 스위트 400개(회귀 테스트)를 돌려 확인했다 — 실패 1건(`UserApplicationFlowTest`)은 위에 기록한 대로 기존 결함.
