...
- 작성자: Claude
- 브랜치: main
- 커밋·push: `86203af`까지 push 완료(사용자 요청으로 그 시점에 push). 그 이후(관리자 카드 개별 다운로드, 관리자 상태 전이 자동화 백엔드+프론트) 신규 변경은 아직 push 요청 없었음.

## 현재 워킹 트리

**미커밋 상태** — 아래 "완료 — 관리자 상태 전이 자동화(백엔드+프론트)" 절의 프론트 소스·`docs/collab/TODO.md`/`CHANGELOG.md` 변경이 아직 커밋되지 않았다(백엔드는 이미 별도 커밋됨, 아래 참고). `tsc --noEmit`/`npm run build` 통과, dev 컨테이너 Playwright로 실제 확인 완료. 다음 세션은 커밋부터 시작하면 된다.

## 완료 — 관리자 상태 전이 — 드롭다운 제거, 업무 버튼 + 자동 전이 (2026-09-24 정책 확정, 백엔드+프론트 구현 완료)
...
- **다음에 할 일**: 위 프론트 변경 + 문서 갱신을 커밋(아직 안 함).
