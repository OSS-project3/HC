...
- 작성자: Claude
- 브랜치: main
- 커밋·push: 아래 "완료" 항목 전부 로컬 커밋 완료. **push는 하지 않음**(2026-09-21에 그 전까지의 전체 세션 작업을 이미 push·배포했고, 그 이후 신규 작업은 로컬에만 있음 — 정확한 ahead 커밋 수는 `git log origin/main..HEAD --oneline`으로 확인할 것).

## 현재 워킹 트리

이 문서 갱신 시점 기준 코드·테스트·관련 문서는 커밋 완료(push만 안 함). `docs/collab/CHANGELOG.md`/`docs/collab/TODO.md`는 이 HANDOFF 갱신과 함께 커밋 예정.

## 진행 중 — 십이간지·카드 디자인 선택 이미지 미리보기 (2026-09-22 Codex 확정 정책)

- **백엔드 완료**(2026-09-23, 커밋 `3336b6f`) — 아래 15번 항목 참고. 20개 신규 테스트 GREEN, 전체 회귀 무관 플레이키 1건 제외 통과.
- **프론트엔드는 아직 시작 안 함** — 사용자가 "구현 진행하되 백엔드부터 구현해줘"라고 명시적으로 순서를 지정했으므로, 다음 세션은 프론트 착수 전에 반드시 사용자에게 진행 확인을 받을 것(이 세션 전체에서 일관되게 지켜온 패턴). 프론트 체크리스트·구현 순서는 `docs/collab/TODO.md` "십이간지·카드 디자인 선택 이미지 미리보기" 절의 "Frontend"/"구현 순서와 검증" 4~7단계 그대로 유효.
...
16. **십이간지·카드 디자인 선택 이미지 미리보기 — 백엔드 구현**. 위 15번 검증 커밋(`c4db930`)을 위해 `TODO.md`를 다시 열었다가, Codex가 같은 파일에 커밋 없이 직접 써둔 완전히 새로운 대형 기능 스펙("십이간지·카드 디자인 선택 이미지 미리보기", 2026-09-22 확정)을 발견 — 세션 초반 사용자가 "십이간지 캐릭터나 카드 디자인 셀렉시 토글 선택할 때 옆에 포인터를 갖다대면... 이미지가 떴으면 좋겠다"고 요청했던 바로 그 기능이었다. 사용자에게 보고 후 "구현 진행하되 백엔드부터 구현해줘" 지시를 받아 진행 보드를 먼저 claim(커밋 `e098d6b`), 이어서 백엔드 구현 완료(커밋 `3336b6f`) — 상세는 위 "진행 중" 절과 `docs/collab/CHANGELOG.md` 2026-09-23 항목 참고. (2026-09-23)

## 검증

- 백엔드: 항목별 개별 회귀 + 세션 마지막 전체 회귀 실행, 마지막 실행(2026-09-23, 십이간지·카드 디자인 미리보기 백엔드 포함)은 1001개 중 1000개 통과(무관 플레이키 `HighSchoolSeederIntegrationTest.reseedingTheSameCsvDoesNotDuplicateRows` 1건만 실패, 단독 실행 시 통과 재확인) — 스위트 전체가 공유하는 `schools` 테이블을 여러 테스트가 각자 `deleteAll()`해서 생기는 기존 문제, 이 세션이 유발한 게 아님(재현·원인 확인은 `qa_state_persistence_checklist.md` 10번 항목 참고).
...
6. **십이간지·카드 디자인 선택 이미지 미리보기 — 프론트엔드 미착수**. 백엔드(API 2개, 서비스, 테스트, 문서)는 완료(위 16번, 커밋 `3336b6f`)했지만 사용자가 "백엔드부터"라고 명시했으므로 프론트(`ZodiacDesignSelector.tsx`/`CardDesignSelector.tsx`/`api.ts`/`ApplicationDetail.tsx`/`CardProductionPanel.tsx`/`AdminPage.css`)는 손대지 않았다. 다음 세션은 사용자의 진행 확인을 받은 뒤 `docs/collab/TODO.md`의 Frontend 체크리스트·구현 순서 4~7단계를 그대로 따를 것.
