...
- **✅ Event 협업 로고·회사명·관리자 전체목록·갤러리 편집 구현 완료(Claude, 2026-08-21, 커밋 `ea1a41f` 구현 + `433993b` 테스트)**: `EventPost`에 `companyName`/`logoImagePath`(`COLLABORATION` 전용, `BOOTH`가 보내면 `INVALID_INPUT`) 추가 — 엔티티가 `assertCollaborationInvariant()`로 최종 상태를 스스로 검증해 호출 순서에 안 흔들리게 설계함. 생성 API에 `logo` multipart part 추가, 수정 API에 `removeLogo`/`removeThumbnail`(유지·교체·삭제)와 `keepImageIds`(생략=전체유지, `[]`=전체삭제, 그 외=지정 순서로 재정렬+신규 이미지 추가, 타 Event 이미지 ID 거절) 갤러리 편집 추가. 관리자 전체목록·상세(`GET /api/admin/events`, `/{id}`, `visible` 무관) 신규. 공개 응답에도 `companyName`/`logoImageUrl` 추가(기존 필드는 안 건드려 하위 호환).
  ...
  - **⚠️ 프론트 필드명 불일치 발견(구현 후 정적 대조로 확인)**: 백엔드 응답은 `companyName`/`logoImageUrl`(기존 `thumbnailImageUrl` 명명 규칙과 통일)인데, 프론트 `FeedPost`(`data/eventFeedPosts.ts:10-11`)는 `company`/`logoUrl`을 쓴다. 백엔드 필드명을 바꾸지 않고, 이미 있는 `eventToFeedPost()` 어댑터(`thumbnailImageUrl→image` 변환과 동일 패턴)에 매핑 2줄만 추가하면 되는 것으로 판단 — `FRONTEND_API_GAPS.md` §1.6에 기록.
