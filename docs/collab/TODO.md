...
| 🔵 | Inquiry(1:1 문의) 도메인 신규 구현 — INQUIRY-1~3 완료(2026-08-19) | Claude | `main` | **`docs/specs/inquiry/requirements.md`(source of truth)** §⑨ 구현 체크리스트 | 정책·체크리스트 전부 확정(§①~⑨). **INQUIRY-1**(`POST /api/inquiries`, `1abab25`)·**INQUIRY-2**(`GET /api/my/inquiries`, `/{id}`, `0b08b41`)·**INQUIRY-3**(`GET /api/admin/inquiries`, `/{id}` — `listAdmin`/`getAdminDetail`, `InquiryAdminController`, 라우트 레벨 권한 강제만·서비스 재검증 없음(Board 선례), `3cb647f`) 구현+테스트 완료. 신규 테스트 누적 23개 전부 통과, 전체 스위트 459개 중 기존과 동일하게 `UserApplicationFlowTest.fullUserApplicationFlow`(pre-existing) 1건만 실패(회귀 없음). 다음 단위: **INQUIRY-4**(`PATCH .../answer` — 답변 등록+이메일 알림). |
