...
| 🔵 | Task 1. `ApplicationService` 생성 로직 메서드 추출 (Factory/Context 이전 단계) | - | `feature/application-domain-impl` | `domain/application/service/ApplicationService.java` | `createIndividualApplication`/`createIndividualApplicant`/`createIndividualMember` 등으로 이미 추출 진행 중. 동작은 그대로 유지 |
| ⚪ | Task 2. Factory + Context — Application 생성 책임 분리 | Codex | - | `domain/application/service/ApplicationService.java` | `ApplicationFactory`, `CreatedApplication`, `IndividualCreationContext` 신설, 생성 로직을 그쪽으로 이동. Service는 흐름만 담당. 이 단계도 동작은 그대로 유지 |
| ⚪ | Task 3. Validation 통합 | Codex | - | `domain/application/service/ApplicationService.java` | 회원존재/ACTIVE/USER권한/약관동의/하루3회 제한/PhotoValidator/StudentCardValidator — 전부 "Validation"이라는 하나의 관심사라 PR을 ACTIVE→약관→사진→학생증처럼 잘게 쪼갤 필요 없음. 단, PhotoValidator 등 개별 Validator가 1000줄급으로 커지면 그건 별도 PR |
| ⚪ | Task 4. 운영 개선 | Codex | - | `domain/application/service/ApplicationService.java` | 신청번호 Generator, 동시성 처리, 파일 보상 처리(업로드 실패 롤백 등), Retry, Transaction 경계 정리 — 운영성 개선이라 Task 2/3 이후 마지막 순서 |
