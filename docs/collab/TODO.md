...
| 🔵 | Inquiry(1:1 문의) 도메인 신규 구현 — INQUIRY-1 완료(2026-08-19) | Claude | `main` | **`docs/specs/inquiry/requirements.md`(source of truth)** §⑨ 구현 체크리스트 | 정책·체크리스트 전부 확정(§①~⑨). **INQUIRY-1**(`POST /api/inquiries`, `Inquiry` 엔티티/Repository/`InquiryService`/`InquiryController`, `privacyConsent` 서버 검증 포함) 구현+테스트 완료(커밋 `1abab25`) — 신규 테스트 7개(`InquiryTest` 2, `InquiryServiceTest` 1, `InquiryControllerTest` 4) 전부 통과, 전체 스위트 443개 중 기존과 동일하게 `UserApplicationFlowTest.fullUserApplicationFlow`(pre-existing) 1건만 실패(회귀 없음). 다음 단위: **INQUIRY-2**(`GET /api/my/inquiries`, `/{id}`). |
