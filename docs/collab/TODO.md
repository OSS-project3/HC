...
| ⚪ | Inquiry(1:1 문의) 도메인 신규 구현 | 미정 | `main` | `docs/FRONTEND_API_GAPS.md` §1.3, `docs/FRONTEND_USER_FLOW_AUDIT.md`(Codex, 2026-08-14) "공지·문의·행사" | 백엔드에 도메인 자체가 없음(`Inquiry` 엔티티/Controller 전무) — Codex 감사에서도 동일 지적, `InquiryPage`/`InquiryDetailPage`/`MyPage`/`AdminPage`가 전부 `localStorage["customer-inquiries"]` 사용 확인. 데이터 계약(`InquiryRecord`): `category, name, email, phone, title, content, status(PENDING/COMPLETED), answer, answeredAt`. ✅ 비회원 허용 여부는 2026-08-19 코드 확인으로 해소됨(`InquiryPage.tsx`가 로그인 필수로 이미 구현돼 있어 `POST /api/inquiries`는 USER 전용으로 설계) — 더 이상 미결정 아님. `Inquiry.userId`는 `RULES.md` §3 원칙대로 JWT에서 추출, 요청 바디로 받지 않음(프론트의 이메일 매칭 방식 재현 안 함). ✅ **서비스 흐름·API 명세도 2026-08-19 확정**(사용자 제시안 검토 완료, `docs/FRONTEND_API_GAPS.md` §1.3 참고) — `POST /api/inquiries`, `GET /api/my/inquiries`(+`/{id}`), `GET /api/admin/inquiries`(+`/{id}`), `PATCH /api/admin/inquiries/{id}/answer`(답변 등록/수정, 성공 시 `COMPLETED` 전이), `PATCH /api/admin/inquiries/{id}/status`(답변과 무관하게 상태만 독립 변경 — `AdminPage.tsx`의 별도 상태 드롭다운 유지 목적으로 신설) 총 6개. 착수 시 바로 구현 가능한 상태. |
