...
| 🔵 | 회원탈퇴 정책 변경 — 소프트 삭제(익명화) → 즉시 하드 삭제 — WITHDRAW-1~3 완료(2026-08-19) | Claude | `main` | **`docs/collab/user.md`(source of truth)** §19 구현 체크리스트(WITHDRAW-1~5), `arch.md` §4.1 "탈퇴 정책"(요약), `backend/FRONTEND_API_REQUIREMENTS.md` §3, `docs/api/user.md` API 4 | 사용자 요청으로 스코프 분석 먼저 수행 — `arch.md` §5.1(FK 없는 Long 참조)이 전역 적용돼 있어 `User` row를 하드 삭제해도 Application/Review/Board/Inquiry는 전부 스냅샷 저장이라 화면 표시가 깨지지 않음을 확인. 사용자가 정책표로 1차 확정한 뒤, 사용자가 별도로 작성해둔 상세 정책 원본(`docs/collab/user.md`)을 발견해 그 기준으로 `arch.md` 등을 재조정(상품 수령일 기산점, 법정대리인 조항, Review 표시명 익명화는 미확정, 파기 감사로그 형식 등 보강). 확정된 정책 요약: 탈퇴 즉시 확정(유예기간·자동복구 폐지), `User`/`RefreshTokenSession`/`ApplicationDailyLimit`만 하드 삭제, `Application`(+`Applicant`/`Member`/`Receiver`)·결제이력·`Inquiry`·`Review`·`Board.created_by_user_id`·`AdminActivityLog.admin_id`는 삭제하지 않고 각자 보존정책 유지, 동일 이메일 재가입 가능하나 과거 데이터 자동 승계 안 함, 익명화 스케줄러 제거. **WITHDRAW-1**(OAuth 재로그인 자동복구 제거, `a3bc798`)·**WITHDRAW-2**(일반 로그인 자동복구 제거 + `restored` 응답 필드 폐지, `b83bd65`)·**WITHDRAW-3**(익명화 스케줄러+`anonymize()`/`isRestorable()`/`restore()` 제거, `f956120`) 구현+테스트 완료. 착수 전 전체 참조 검색 결과 `status` 완전 제거가 `ApplicationService`/`ReviewEligibilityService`까지 걸치는 걸 확인해 **WITHDRAW-3B**(`UserStatus`/`status`/`isWithdrawn()` 완전 제거, 사용자 확정)로 분리(`docs/collab/user.md` §19). 다음 단위: **WITHDRAW-3B** → **WITHDRAW-4**(`UserService.withdraw()`를 하드 삭제로 교체, 핵심 단위). 개인정보처리방침 "회원가입 정보: 상품 수령 후 6개월" 문구와 즉시 하드 삭제 정책의 관계는 2026-08-19 해소됨(`docs/collab/user.md` §17.1) — 6개월은 미탈퇴 활동 회원의 기본 보유기간이고, 탈퇴 시에는 그 기간을 기다리지 않고 지체 없이 파기(둘 중 먼저 오는 사유로 파기). 남은 미해결은 §17.2(비밀번호 제3자 제공 문구)·§17.3(제3자 제공/처리위탁 혼재)뿐. |
