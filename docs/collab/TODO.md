...
- [ ] `CardFieldDefinition`(record) + 카드종류별 필드 좌표 상수 — 우선 명예한국인증 1개로 좌표 변환식부터 역산
- [ ] `CardImageCompositor` 서비스 — 배경+로고+직인+사진 합성, 텍스트 렌더링(폰트 적용)
- [ ] 렌더링 결과를 `시안_최종.jpg`와 육안 대조해 좌표 변환식 보정(반복 작업, 분석적으로 못 함)
- [ ] 명예한국인증 확인되면 명예시민증/방문증도 동일 패턴 적용(방문증은 PORTRAIT 치수 주의)
- [ ] 파일명 불일치 6건 별칭 매핑 또는 스킵 처리, 사용자에게 목록 보고
- [ ] 카드번호 생성(무작위+유일성) — `ApplicationMember`에 필드 없으면 추가
