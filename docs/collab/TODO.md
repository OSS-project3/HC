...
- [x] **AUTH-4** 이메일 회원가입 API (`POST /api/auth/signup`) — ✅ 구현+테스트 완료(Claude, 2026-08-19, 미커밋)
  ...
  - 확정 내용 반영: `backend/FRONTEND_API_REQUIREMENTS.md` §3(회원가입 요청 예시에 `signupToken` 추가, 필수 필드 5개·비밀번호 정책 명시), `docs/api/auth.md`(API 4/5/6으로 SIGNUP-1/SIGNUP-2/AUTH-4 신규 문서화, 총 개수 `/3`→`/6`), `SignupRequest.java`(DTO Validation, 위 내용), 프론트 `SignupPage.tsx`(비밀번호 검증을 8~64자+복잡도 조합 요구 → 8~72자만 요구로 변경, 힌트 문구 갱신 — `phone`은 이미 필수였으므로 검증 로직 변경 없음).
