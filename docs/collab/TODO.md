...
| 🔵 | Inquiry(1:1 문의) 도메인 신규 구현 — INQUIRY-1·2 완료(2026-08-19) | Claude | `main` | **`docs/specs/inquiry/requirements.md`(source of truth)** §⑨ 구현 체크리스트 | 정책·체크리스트 전부 확정(§①~⑨). **INQUIRY-1**(`POST /api/inquiries`, 커밋 `1abab25`)·**INQUIRY-2**(`GET /api/my/inquiries`, `/{id}` — `InquiryListItemResponse`/`InquiryDetailResponse`, 404→403 소유권 분리, `MyInquiryController`, 커밋 `0b08b41`) 구현+테스트 완료. 신규 테스트 누적 16개 전부 통과, 전체 스위트 452개 중 기존과 동일하게 `UserApplicationFlowTest.fullUserApplicationFlow`(pre-existing) 1건만 실패(회귀 없음). 다음 단위: **INQUIRY-3**(`GET /api/admin/inquiries`, `/{id}`). |
