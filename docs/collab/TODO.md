...
| 🔵 | Inquiry(1:1 문의) 도메인 신규 구현 — INQUIRY-1~4 완료(2026-08-19) | Claude | `main` | **`docs/specs/inquiry/requirements.md`(source of truth)** §⑨ 구현 체크리스트 | 정책·체크리스트 전부 확정(§①~⑨). **INQUIRY-1**(`1abab25`)·**INQUIRY-2**(`0b08b41`)·**INQUIRY-3**(`3cb647f`)·**INQUIRY-4**(`PATCH .../answer` — `EmailType.INQUIRY_ANSWERED`, `InquiryAnswerRequest`, `InquiryService.answer`가 갱신 직전 `answer==null` 판정으로 최초 등록만 커밋 후(`TransactionSynchronizationManager`) best-effort 이메일 발송·수정 시엔 재발송 안 함·`EmailSender` 실패해도 답변 저장 유지, `f877d2d`) 구현+테스트 완료. RULES.md §8 신규 정책에 따라 이번 단위는 도메인 내부 변경이라 전체 스위트 대신 Inquiry 범위 테스트만 실행(31개 전부 통과) — 직전 전체 스위트 기준(459개, pre-existing 실패 1건)에서 공통 인프라 변경 없음. 다음 단위: **INQUIRY-5**(`PATCH .../status`, 마지막 단위). |
