...
| ⚪ | Inquiry(1:1 문의) 도메인 신규 구현 | 미정 | `main` | **`docs/specs/inquiry/requirements.md`(source of truth, 2026-08-19 신설)**, `docs/FRONTEND_API_GAPS.md` §1.3(요약만), `docs/FRONTEND_USER_FLOW_AUDIT.md`(Codex, 2026-08-14) | 백엔드에 도메인 자체가 없음(`Inquiry` 엔티티/Controller 전무). 서비스 흐름·API 6개(`POST /api/inquiries`, `GET /api/my/inquiries`+`/{id}`, `GET /api/admin/inquiries`+`/{id}`, `PATCH .../answer`, `PATCH .../status`)·핵심 정책(로그인 필수, `userId`는 JWT에서만 추출, 검색 API 미포함, 제출 후 수정·삭제 불가)까지 확정 완료 — `docs/specs/inquiry/requirements.md`에서 계속 정책을 다듬는 중. 착수 시 바로 구현 가능. |
