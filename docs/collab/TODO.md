...
| ⚪ | Inquiry(1:1 문의) 도메인 신규 구현 | 미정 | `main` | **`docs/specs/inquiry/requirements.md`(source of truth, 2026-08-19 신설)**, `docs/FRONTEND_API_GAPS.md` §1.3(요약만), `docs/FRONTEND_USER_FLOW_AUDIT.md`(Codex, 2026-08-14) | 백엔드에 도메인 자체가 없음(`Inquiry` 엔티티/Controller/Repository/Service 전무 — `common/enums/InquiryCategory.java`만 선반영, 아래 참고). 서비스 흐름·API 6개·핵심 정책(로그인 필수, `userId`는 JWT에서만 추출, 검색 API 미포함, 제출 후 수정·삭제 불가, `name`/`email`/`phone`은 요청 바디, 소유자 아닌 자원은 404) **전부 확정 완료**(`docs/specs/inquiry/requirements.md` §①~⑥). 남은 미결정 사항 없음(category enum/스팸방지/재문의/첨부파일 4건 전부 2026-08-19 해소) — 착수 시 바로 구현 가능. |
