...
- 관리자가 `REVIEWING` 신청을 사진 반려하면 신청 전체가 `PHOTO_REJECTED`로 전이되고 `photoRejectReason`과 관리자 활동 로그가 저장된다.
- 사용자 재업로드 API `PATCH /api/applications/{applicationId}/photo`는 이미 구현되어 있다. 로그인한 신청 소유자만 호출할 수 있고 `PHOTO_REJECTED` 상태에서만 허용한다.
- 개인 신청은 `photo` 1장, 단체 신청은 수정된 Excel과 모든 사진을 포함한 `submitFile` ZIP 전체를 받는다.
- 재업로드 성공 시 `PHOTO_REJECTED → REVIEWING`, `photoRejectReason = null`로 처리한다. 새 파일 및 DB 저장 실패 시 기존 파일·상태를 유지하고, 성공 시 DB 커밋 후 기존 S3 파일을 삭제하는 보상/정리 로직도 이미 구현되어 있다.
- 마이페이지 상세는 상태·신청 유형·반려 사유를 이미 내려받지만 사유만 표시하고 재업로드 UI가 없다.
- 기존 `/mobile-card`의 재업로드 UI는 `/lookup`이나 마이페이지에서 진입시키는 링크·상태 전달이 없어 실제 사용자 경로로 접근할 수 없다. 이번 작업은 마이페이지에서 직접 재업로드하도록 연결한다.

### 확정 정책

1. 재업로드 UI는 마이페이지 `제작 내역 → 신청 상세` 안에서 제공한다. 별도 페이지로 이동시키지 않는다.
2. `detail.status === PHOTO_REJECTED`인 신청에만 반려 사유와 재업로드 영역을 표시한다.
3. 파일을 선택하는 즉시 업로드하지 않는다. 선택 파일명·크기를 확인한 후 별도 `재업로드` 버튼을 눌러 제출한다.
4. 개인 신청은 얼굴 사진 1장만 다시 받는다.
   - multipart part: `photo`
   - 허용 형식: JPG/JPEG/PNG(`image/jpeg`, `image/png`)
   - 최대 크기: 5 MiB
   - WebP는 허용하지 않는다.
5. 단체 신청은 수정된 Excel과 구성원 사진을 포함한 ZIP 전체를 다시 받는다. 특정 구성원 사진 한 장만 교체하는 UI는 만들지 않는다.
   - multipart part: `submitFile`
   - 허용 형식: ZIP
   - 최대 크기: 250 MiB
   - 기존 단체 Excel/사진 번호 매핑 및 전체 실패 정책을 그대로 적용한다.
6. 학생증 개인 신청도 얼굴 사진만 교체한다. 학교 로고·학교 직인은 이 화면에서 다시 받지 않는다.
7. 업로드 중에는 파일 입력과 제출 버튼을 비활성화해 중복 제출을 막고 처리 중임을 표시한다. 퍼센트 업로드 진행률은 이번 범위가 아니다.
8. 성공하면 응답 상태(`REVIEWING`)를 목록에 즉시 반영하고 펼친 상세를 다시 조회한다. 반려 사유와 재업로드 UI가 사라졌는지 서버 응답 기준으로 확인한다.
9. 실패하면 기존 신청 상태와 선택 파일을 유지하고 `ApiError.message`를 표시한다. 단체 검증 실패의 `errors[]`가 있으면 `row/field/code/message`를 사용자가 확인할 수 있게 표시한다.
10. `PHOTO_REJECTED`에서 제공되는 기존 `신청 취소` 기능은 그대로 유지한다. 재업로드와 취소는 서로 다른 명시적 버튼이다.
11. 사진 반려 이메일·푸시 알림, 관리자 반려 UI 변경, 백엔드 상태 전이·파일 처리 변경은 이번 범위가 아니다.
...
## 관리자 상태 전이 — 드롭다운 제거, 업무 버튼 + 자동 전이 (2026-09-24 정책 확정, 백엔드+프론트 구현 완료)

상태: ✅ 완료(Claude) — 백엔드·프론트 모두 구현·검증 완료.

### 배경 — 조사 결과 요약

`ApplicationDetail.tsx`(167-209행)의 "상태 변경" `<select>`는 실제로는 임의 상태를 고르는 게 아니라 이미 이름 붙은 8개 업무 액션(결제확인/검토시작/작명승인/사진반려/작명완료처리/제작시작/카드발급완료/배송발송)을 현재 상태에 따라 필터링해 드롭다운으로만 보여주는 것이다. 각 액션은 이미 독립된 API 엔드포인트다.

- `completeNaming()`(`ApplicationService.java:722-735`)은 이미 `validateNamingComplete()`로 전 멤버 성씨·이름·뜻을 검증한 뒤에만 `NAME_EDITING→PRODUCTION_READY` 전이한다.
- `startProducing()`(`:689-697`)은 이미 `requireCardGenerationComplete()`(`:718-739`)로 전 멤버 카드 생성 여부를 검증한 뒤에만 `PRODUCTION_READY→PRODUCING` 전이한다(`markCardReady()`도 같은 검증 재사용).
- 멤버별 "이름 확정"(`assignMemberName`, `:546-`)과 "카드 생성"(`CardGenerationService.generate()`)은 **오늘도 Application 상태를 전혀 바꾸지 않는다** — 그 멤버 한 명의 데이터만 저장한다.
- `requireCardGenerationComplete()`는 현재 검증 실패 시 바로 `throw`하는 형태라, "자동 전이 여부 확인용"(실패해도 예외 없이 그냥 넘어가야 함)으로 재사용하려면 "에러 리스트 반환" 책임과 "그 리스트가 비어있지 않으면 throw" 책임을 분리하는 리팩터링이 필요하다(`validateNamingComplete`는 이미 이 형태라 그대로 재사용 가능).

### 확정 정책

- **버튼 자체(이름 확정/카드 생성)는 그대로 둔다** — 위치·동작 변경 없음, 계속 멤버 카드 안에서 그 멤버 한 명의 데이터만 저장한다.
- **새로 추가되는 것은 그 버튼을 눌렀을 때의 부수효과뿐이다**: 저장 직후 "이제 전원 완료됐는가?"를 확인해서, 그렇다면 백엔드가 **자동으로** 다음 상태로 전이시킨다.
  - 마지막 멤버까지 이름 확정 완료 → 자동으로 `NAME_EDITING → PRODUCTION_READY`(기존 `completeNaming()`과 동일한 검증·전이). 아직 남은 멤버가 있으면 `NAME_EDITING` 유지, 에러 없이 조용히 넘어간다.
  - 마지막 멤버까지 카드 생성 완료 → 자동으로 `PRODUCTION_READY → PRODUCING`(기존 `startProducing()`과 동일한 검증·전이). 아직 남은 멤버가 있으면 `PRODUCTION_READY` 유지.
- 기존 "작명완료 처리"/"제작 시작" **버튼은 프론트에서 제거**하지만, 백엔드 엔드포인트(`POST .../complete-naming`, `POST .../start-producing`)는 **삭제하지 않고 유지**한다 — 자동 전이가 어떤 이유로든 안 걸렸을 때 관리자가 수동으로 복구할 수 있는 경로로 남겨둔다.
...
### Frontend — ✅ 구현 완료(2026-09-24, `tsc`/`build`/실제 dev 컨테이너 Playwright 확인)

- [x] `ApplicationDetail.tsx`의 "상태 변경" `<select>`를 제거하고, 남는 업무 액션(입금확인/검토시작/작명승인/사진반려/카드발급완료/배송발송)을 각각 독립 버튼으로 전환.
- [x] "작명완료 처리"/"제작 시작" 항목은 목록에서 제거(백엔드가 자동 처리, 수동 엔드포인트는 UI에 노출만 안 할 뿐 그대로 존재).
- [x] 전체 파이프라인 단계를 보여주는 토글 형태의 읽기 전용 상태 표시기 신설(`전체 진행 단계 보기` 버튼 → 주 경로 6단계 + 이탈 경로 2개(사진반려/취소) 펼침 표시, 현재 단계 강조).
- [x] 자동 전이가 발생했을 때 관리자에게 어떻게 알릴지 — **사용자 확정(2026-09-24): 별도 알림 추가하지 않음, 기존 저장 성공 토스트("이름을 확정했습니다" 등)만 유지**. 상태 배지·파이프라인 표시기는 다음 상세 재조회(`onSaved`가 이미 매번 호출하는 `reloadDetail`)에서 자동 갱신되므로 추가 구현 불필요.
...
- 확정 정책과 실제 백엔드 검증은 일치한다. 성씨를 제외한 이름은 한글 2~3글자만 허용하고, 한자가 있으면 한글 이름과 한자 이름의 Unicode 글자 수가 같아야 한다(`docs/specs/application/admin-saju.md`, `ApplicationMember.validateNameFormat`). **이 규칙은 완화하지 않는다.**
- 실제 `sajuNames.json` 700건을 위 규칙으로 검사한 결과 문제 후보는 정확히 20건이다.
  - 1글자 이름 14건
  - 4글자 이름 4건
  - 한글 이름과 한자 글자 수가 다른 손상 데이터 2건(`태산`, `현산`)
- 프론트 `frontend/src/data/sajuNames.json`과 백엔드 `backend/honor-citizen/src/main/resources/seed/saju-names.json`은 현재 SHA-256이 같은 동일 복사본이다. 한쪽만 수정해 두 파일을 서로 다른 데이터로 만들지 않는다.
- 현재 추천 계산은 백엔드 DB가 아니라 프론트 번들 `sajuNames.json`을 직접 사용한다. 백엔드의 `saju_names` 테이블은 향후 추천 API 이관을 위한 데이터이며 현재 추천·작명 저장 API에서 조회하지 않는다.
- 백엔드 `SajuNameSeeder`는 `count() > 0`이면 전체 시드를 건너뛴다. 따라서 JSON만 수정하면 신규 DB에는 반영되지만 이미 700건이 들어간 기존 DB의 손상된 두 행은 자동으로 고쳐지지 않는다.

### 확정 정책

1. `ApplicationMember.validateNameFormat()`의 한글 이름 2~3글자 및 한자 글자 수 일치 검증은 그대로 유지한다. API·DTO·Entity 컬럼·ErrorCode도 변경하지 않는다.
2. 1글자 14건과 4글자 4건은 이름 사전 원본에서 삭제하지 않는다. 사전·DB에는 보존하되 **관리자 추천 후보에서만 제외**한다.
3. 추천 가능 조건은 다음 두 조건을 모두 만족하는 항목이다.
   - `name`이 정규식 `[가-힣]{2,3}`을 만족한다.
   - `hanja`가 존재하면 `Array.from(hanja).length == Array.from(name).length`이다. Java의 `codePointCount`와 같은 Unicode 코드포인트 기준으로 판정하며 JavaScript의 UTF-16 `string.length`만 사용하지 않는다.
4. 손상 데이터 두 건은 동일 사전에 이미 존재하는 단독 이름 `산 → 祘`, `셈 산(祘)`을 근거로 다음과 같이 바로잡는다.
   - `태산`: `兌示산` → `兌祘`, `기쁠 태(兌) 셈 산(示산)` → `기쁠 태(兌) 셈 산(祘)`
   - `현산`: `鉉示산` → `鉉祘`, `솥귀 현(鉉) 셈 산(示산)` → `솥귀 현(鉉) 셈 산(祘)`
5. 추천 필터는 반드시 **점수 계산·정렬·상위 5개 절단 전에** 적용한다. 상위 5개를 먼저 뽑은 뒤 잘못된 후보를 제거해 결과가 5개보다 적어지는 구현은 금지한다.
6. 두 한자 오류를 수정하면 추천 가능한 데이터는 682건이고, 길이 정책 때문에 제외되는 데이터는 18건이다. 기본 추천 결과는 계속 5건이어야 하며 동일 만세력 입력에 대한 결정성(score 내림차순 → 원본 index 오름차순)은 유지한다.
...
- 저장이 완료된 이름·카드번호만 반영하며 입력 중 값을 DB에 자동 저장하지 않는다.
- 미리보기 허용 상태는 `NAME_EDITING`, `PRODUCTION_READY`로 제한한다. `PRODUCING` 이후에는 생성된 카드 이미지를 사용한다.
- 이름·카드번호·만세력 등 렌더링 필수값이 모두 저장된 경우에만 호출하며 임시 문구로 대체하지 않는다.
- 이름·카드번호 저장 성공 또는 디자인·발급일·학생증 앞면/뒷면 글씨색 변경 후 800ms debounce를 적용한다.
- 단체 신청은 현재 선택·확장한 구성원 1명만 호출하며 최초 진입 또는 공통값 변경 때 전체 구성원을 일괄 호출하지 않는다.
- 마지막 요청 결과만 반영하고 늦게 도착한 이전 응답은 무시한다.
- 요청 중 기존 이미지를 유지하고 로딩을 표시한다. 실패 시 기존 이미지와 오류·재시도 수단을 함께 제공한다.
- 미리보기는 읽기 전용이며 DB 저장이나 `AdminActivityLog` 기록을 수행하지 않는다.
- 기존 `CardPreviewRequest`·응답 계약과 DB 스키마는 변경하지 않는다.
...
|---|---|---|---|---|---|
| 🔵 | 관리자 상태 전이 — 드롭다운 제거, 업무 버튼 + 자동 전이 | Claude(백엔드 착수) | `main` | 본 문서 "관리자 상태 전이 — 드롭다운 제거, 업무 버튼 + 자동 전이" 절 | 정책 확정(이름확정/카드생성 버튼은 그대로, 마지막 멤버 완료 시 자동 전이), 구현 미착수 |
...
- [x] `CardGenerationServiceTest`(신규, 11개, TODO 초안의 `CardGenerateServiceTest`에서 실제 클래스명 `CardGenerationService`에 맞춰 개명) — **완료(2026-08-30)**: 신규 생성 성공, 재생성 성공(기존 파일 commit 후 삭제 확인), 동일 요청 반복 호출이 멱등 스킵 없이 매번 재실행되는지, 렌더링 실패 시 업로드 없음, front 업로드 성공+back 업로드 실패 시 front만 보상 삭제, 디자인 충돌 시 `CARD_DESIGN_MISMATCH`, 발급일자 충돌 시 `CARD_ISSUE_DATE_MISMATCH`, 상태 게이트(`PRODUCTION_READY` 허용, `PRODUCING && cardReadyAt == null` 허용, `PRODUCING && cardReadyAt != null` 거절, `COMPLETED` 거절), 상태 자동전환 없음, 비관리자 거절. **미커버(2026-08-30 확인)**: "DB 반영 실패 시 두 key 모두 보상 삭제"·"준비 단계 이후 상태가 바뀐 경우 최종 재검증 실패" 2개는 진짜 동시 요청(두 트랜잭션 경합) 재현이 필요해 이번 단위/통합테스트로는 못 만들었다 — `CardPreviewService`와 공유하는 나머지 검증 실패 케이스는 `CardPreviewServiceTest`가 이미 폭넓게 커버해 중복 작성하지 않았다.
- [x] `CardPreviewServiceTest` 보강 — **완료(2026-08-30, 16→19개)**: `Application.cardDesignId`/`cardIssueDate`가 이미 확정된 상태에서 다른 값으로 Preview 요청 시 `CARD_DESIGN_MISMATCH`/`CARD_ISSUE_DATE_MISMATCH`로 거절, 확정 후 같은 값이면 정상 미리보기 되는지까지 확인.
- [x] `ApplicationMemberTest`/`ApplicationTest`: 신규 mutator(`ApplicationMember.assignCardImages`, `Application.confirmCardGeneration`) 단위 테스트 — **완료(2026-08-30)**. 이 코드베이스엔 `ApplicationTest`라는 파일이 없고 `Application` 엔티티 테스트는 `ApplicationStateTransitionTest`에 모여 있어 거기에 추가했다.
- [x] 통합: 실제 Docker+MinIO+curl — **완료(2026-08-30)**. `docker compose build backend` 재빌드 후 실제 fixture(application 7/member 12, 기존 PRODUCTION_READY 데이터)로 `POST .../card-generate` 호출 → 응답 JSON 확인 → DB(`applications.card_design_id/card_issue_date`, `application_members.card_front_path/card_back_path/issue_date`) 실제 반영 확인 → MinIO 버킷(`honorcard-storage-2026` — docker-compose 기본값 `honor-citizen-local`이 아니라 `.env`로 오버라이드된 실제 값, `mc` 컨테이너로 직접 확인) 안에 실제 PNG 2개 존재 확인 → 재생성 curl로 새 파일 생성+기존 파일이 MinIO에서 실제로 사라지는 것까지 확인. `AdminActivityLog.CARD_IMAGE_GENERATED` 실제 기록도 확인. 없는 신청 ID로 curl해 `404 APPLICATION_NOT_FOUND` JSON 에러 포맷도 확인.
...
- [x] **구현 중 발견·수정한 버그 3건**(전부 최소 STUDENT 케이스로만 발현, 다른 3종 무영향):
  ...
  3. **⚠️ `STUDENT_BACK` PORTRAIT 좌표 보정(2026-08-31, 첫 실측 렌더링에서 발견)**: 세로형 뒷면에서 한자가 있는 학생을 렌더링하면 한자 줄과 영문명 줄이 겹쳤다 — 원문 표의 두 줄 간격(2.402mm)이 같은 폰트크기(9pt) 한 줄 높이보다 좁았기 때문(최초 구현 시엔 세로형+한자 조합을 실제로 렌더링해보지 않아 못 잡음). 아래쪽에 여유 공간이 충분해(영문명→풀이 줄 사이 30mm 이상 빔) 영문명 y좌표만 -12.866mm→-9.5mm로 내렸다(간격을 가로형에서 실제로 안 겹치는 값인 5.844mm와 비슷하게 맞춤). 이름/한자/풀이 좌표는 원문 그대로 — 앞면 세로형 생년월일 보정(4-C 앞면 항목 참고)과 같은 성격의 실측 보정.
...
5. **MIME/해상도 검증 실패 에러코드 — ✅ 2026-09-01 결정, 2026-09-01 착수 전 재확인으로 정정**: 크기 초과는 기존 `FILE_TOO_LARGE`(413) 재사용. **MIME 불일치는 새 코드를 안 만든다 — 기존 `UNSUPPORTED_FILE_TYPE`(415)를 그대로 재사용한다**(`EventImageValidator`/`BoardAttachmentValidator` 등 기존 파일 검증기 전부가 이미 이 코드를 씀 — 착수 전 `ErrorCode.java` 재확인으로 발견, 처음 결정 때는 "전용 코드 분리"로만 적었는데 MIME엔 이미 맞는 기존 코드가 있었다). **해상도(비율/최소해상도) 미달만 전용 신규 코드가 필요**하다(예: `CARD_TEMPLATE_INVALID_RESOLUTION` — 기존 4개 파일 관련 코드(`FILE_TOO_LARGE`/`UNSUPPORTED_FILE_TYPE`/`INVALID_IMAGE`/`INVALID_IMAGE_FILE`) 중 "형식은 맞는데 비율·크기가 카드 규격과 안 맞음"에 해당하는 게 없음, 정확한 이름은 구현 시 확정).
...
>
> **⚠️ 동기화 메모(2026-08-31)**: 이 문서는 2026-08-27 작성 이후 실제 구현 진행과 동기화되지 않고 있었다 — 위 4-B/4-C 절에서 이미 완료·`[x]` 표시한 작업(CardDesign 학교 매칭, STUDENT `CardLayouts`/`CardImageCompositor`)이 아래 2-A/2-B에는 여전히 미체크로 남아 있었다. 실제 코드·테스트 상태를 재확인해 아래 항목별로 완료 표시와 근거(파일·테스트명)를 반영했다 — 확인 안 된 항목은 그대로 미체크로 남기고 이유를 적었다(임의로 다 체크하지 않음).
...
- [x] **CardDesign 학교 매칭 구현** — **확인 대상:** `CardDesign`, `CardDesignRepository`, `CardDesignService`, `CardDesignServiceTest`. **재사용:** 기존 cardType·active·orientation 검증과 디자인 응답. **완료 조건:** STUDENT는 `Application.schoolId + Application.orientation`에 맞는 active 디자인 정확히 1개만 반환하고, 비학생 카드는 기존 조회 결과가 변하지 않는다. **✅ 2026-08-30 완료(4-B)** — `CardDesignServiceTest.resolvesStudentDesignBySchoolIdAndOrientation`/`filtersStudentDesignsByActiveToo`/`returnsEmptyListWhenApplicationHasNoSchoolIdYet`로 확인.
- [x] **STUDENT 하드 거절 제거** — **확인 대상:** `CardDesignService`의 `UNSUPPORTED_CARD_TYPE` 분기. **재사용:** 기존 `CARD_DESIGN_NOT_FOUND`. **완료 조건:** 등록 템플릿은 조회되고, schoolId 미연결 또는 템플릿 미등록 상태는 신청·작명을 막지 않으며 디자인 조회·Preview에서만 `CARD_DESIGN_NOT_FOUND`가 발생한다. **✅ 2026-08-30 완료(4-B)** — `CardDesignService.java`에 `UNSUPPORTED_CARD_TYPE` 분기 자체가 더 이상 없음(2026-08-31 재확인, grep 결과 0건). `rejectsStudentCardTypeWithoutApplicationId`/`rejectsStudentCardTypeWithUnknownApplicationId` 테스트로 나머지 거절 케이스 확인.
- [ ] **디자인 학교 불변조건 검증** — **확인 대상:** `CardDesign` 생성/활성화 메서드와 Repository 테스트. **재사용:** 기존 카드종류·방향 불변조건. **완료 조건:** STUDENT 디자인은 schoolId가 필요하고 비학생 디자인은 schoolId가 null이며, 같은 학교+방향의 활성 디자인 2개와 다른 학교·방향 디자인 선택을 모두 차단한다. **⚠️ 미완료(2026-08-31 재확인)** — `CardDesign.create(...)`는 schoolId를 그냥 받아서 저장할 뿐, "STUDENT면 필수/비STUDENT면 null" 강제나 "같은 schoolId+orientation 활성 디자인 2개 차단"을 Entity/Service/DB 레벨에서 강제하는 코드가 없다(unique 제약도 없음). 4-B 절 정정 메모에 이미 적어뒀듯 4-D 업로드 API가 "있으면 UPDATE, 없으면 CREATE"로만 동작해 정상 경로로는 2개가 안 생기지만, 이건 절차적 우회일 뿐 이 항목이 요구하는 명시적 불변조건 검증은 아니다 — 4-D 구현 시 같이 판단.
- [x] **렌더링 입력 최소 확장** — **확인 대상:** `CardMemberData`, `CardPreviewService`, `Application`, `ApplicationMember`. **재사용:** 기존 이름·영문명·사진·카드번호·주소·발급일자·로고·직인·띠 입력. **완료 조건:** schoolType, schoolName, studentId, department, 학생증 표시용 생년월일, orientation을 실제 DB 값으로 전달하고 기존 세 카드 렌더링 결과를 깨지 않는다. **✅ 2026-08-31 완료(4-C)** — `CardMemberData`에 7개 필드 추가(기존 13-인자 생성자 하위 호환 유지), 비학생 3종 테스트 전부 그대로 통과 확인.
- [x] **학생증 템플릿 로딩 경로 확인** — **확인 대상:** `UploadFile`, `StorageService`, `CardDesign.templateFrontId/templateBackId`, STUDENT 리소스/운영 등록 방식. **재사용:** 기존 S3 template 다운로드. **완료 조건:** 개발 탐색용 로컬 파일 경로가 운영 API에 하드코딩되지 않고, 디자인 row가 가리키는 앞·뒷면 template을 사용한다. **✅ 2026-08-31 완료(4-C)** — `CardRenderPreparation.studentMemberData()`가 `design.getTemplateFrontId()/getTemplateBackId()`를 기존 `downloadUploadFile()`로 내려받는다(로컬 경로 하드코딩 없음). 단, 실제로 이 경로를 채울 `CardDesign` row 자체는 4-D가 없어 아직 운영에서 안 만들어짐 — "로딩 코드"는 완료, "실사용 가능 여부"는 4-D 대기.
- [x] **확정 레이아웃만 등록** — **확인 대상:** `CardLayouts.FRONT/BACK`, `CardImageCompositor`, 확정 STUDENT 좌표 자료. **재사용:** 기존 텍스트 축소 최대 2px, 사진·로고·직인 합성 primitive. **완료 조건:** 고등학교/대학교 × 가로/세로 4개 조합을 기존 확정값 그대로 등록하며 좌표·폰트·배치를 새로 설계하거나 임의 보정하지 않는다. **✅ 2026-08-31 완료(4-C), ⚠️ 단 하나 예외 있음** — 원문 확정값을 그대로 등록했으나, `STUDENT_BACK` PORTRAIT의 영문명 y좌표 하나는 실제 렌더링(한자 있는 세로형)에서 옆 줄과 겹치는 게 발견돼 예외적으로 보정했다(4-C 버그 3번 항목, `f907500` 참고) — "임의 보정하지 않는다"는 이 조건과 형식적으로는 어긋나지만, 실측 겹침을 고치지 않으면 렌더링 자체가 깨지므로 앞면 세로형 생년월일 보정(이미 있던 선례)과 같은 성격의 불가피한 보정으로 판단했다.

**2-A 테스트:** `CardDesignServiceTest` → CardDesign Entity/Repository 관련 테스트 → `CardImageCompositorTest`의 비학생 회귀 → `compileJava`. **✅ 2026-08-31 실행 완료** — 전부 통과(위 항목별 근거 참고). CardDesign Entity/Repository의 "학교 불변조건" 전용 테스트만 없음(위 미완료 항목과 동일 이유).

#### 2-B. 확정 DB 데이터로 학생증 Preview 생성 — 카드 렌더링

- [ ] **기존 Preview API 재사용** — **확인 대상:** CardPreviewRequest, CardPreviewService, AdminApplicationController. **재사용:** cardDesignId·issueDate 요청(⚠️ 2026-08-27 계약 변경으로 `side` 필드는 없어지고 응답이 앞/뒤를 한 번에 반환하도록 바뀜, `ApiResponse<CardPreviewResponse>` JSON+base64 — 2-C 절 참고), 관리자 권한, PRODUCTION_READY, Member 소속, 카드번호·발급일자·작명·만세력 검증. **완료 조건:** 학생증 전용 Preview API를 만들지 않고 기존 엔드포인트가 STUDENT도 처리한다. **⚠️ 구조적으로는 충족(2026-08-31)** — `CardRenderPreparation`이 Preview/Generate 공통 진입점이고 STUDENT는 그 안에서 `cardType==STUDENT` 분기로만 처리된다(별도 엔드포인트·Controller 없음). 하지만 이를 실제로 확인하는 `CardPreviewServiceTest`/`AdminApplicationControllerTest`의 STUDENT 케이스가 아직 없다(둘 다 grep 결과 STUDENT 미언급) — 미체크 유지, 자동 테스트로 아직 안 막혀 있다는 뜻.
- [ ] **실제 DB 값 매핑 검증** — **확인 대상:** CardPreviewService의 CardMemberData 구성부와 CardImageCompositor. **재사용:** 저장된 최종 데이터 우선 원칙. **완료 조건:** Excel 저장값, 확정 성씨·이름·한자·뜻, photoPath, 관리자 카드번호, 발급일자, Application.schoolName 스냅샷, 학번·학과, 로고·선택 직인, 띠 결과가 요청 임시값이나 현재 School.name이 아니라 DB 신청 스냅샷에서 전달된다. **⚠️ 미완료(2026-08-31 재확인)** — `CardRenderPreparation.studentMemberData()`가 실제 `Application`/`ApplicationMember` 필드에서 값을 읽어오도록 코드는 짜여 있지만, 이 매핑 자체를 검증하는 자동 테스트(`CardRenderPreparationTest` 등)가 없다. 이번 세션의 육안 검증은 손으로 만든 `CardMemberData`를 직접 넣은 것이라 이 항목(DB→CardMemberData 매핑)의 증거가 되지 못한다 — 별도로 필요.
- [x] **고등학교 × 세로 앞·뒷면 검증** — **확인 대상:** STUDENT HIGH_SCHOOL PORTRAIT layout과 CardImageCompositorTest. **재사용:** 공통 이미지 합성·빈 한자 영역 처리. **완료 조건:** 생년월일 등 고등학교 확정 필드는 표시되고 학번·학과는 표시되지 않으며 앞·뒷면 PNG가 정상 생성된다. **✅ 2026-08-31 완료** — `composesStudentHighSchoolPortraitFrontAsValidPng`(구조) + 이번 세션 실제 렌더링 육안 확인(잘림·겹침 없음, 배경은 디자이너 실물 사용/뒷면은 흰 캔버스).
- [x] **고등학교 × 가로 앞·뒷면 검증** — **확인 대상:** STUDENT HIGH_SCHOOL LANDSCAPE layout. **재사용:** 동일. **완료 조건:** 세로형과 같은 DB 값이 가로형 확정 좌표에 렌더링되며 잘림·사진 겹침이 없다. **✅ 2026-08-31 완료** — `composesStudentHighSchoolLandscapeFrontAsValidPng` + 육안 확인.
- [x] **대학교 × 세로 앞·뒷면 검증** — **확인 대상:** STUDENT UNIVERSITY PORTRAIT layout. **재사용:** 동일. **완료 조건:** 학번·학과가 표시되고 고등학교 전용 표시값은 노출되지 않으며 앞·뒷면 PNG가 정상 생성된다. **✅ 2026-08-31 완료** — `composesStudentUniversityPortraitFrontAsValidPng` + 육안 확인. 이 조합의 뒷면(한자 있음)에서 실제로 영문명↔한자 겹침 버그를 발견·수정(`f907500`).
- [x] **대학교 × 가로 앞·뒷면 검증** — **확인 대상:** STUDENT UNIVERSITY LANDSCAPE layout. **재사용:** 동일. **완료 조건:** 학번·학과를 포함한 실제 DB 값이 가로형 확정 좌표에 렌더링되며 잘림·사진 겹침이 없다. **✅ 2026-08-31 완료** — `composesStudentUniversityLandscapeFrontAsValidPng` + 육안 확인.
- [x] **학생증 선택 직인·한자 없음 검증** — **확인 대상:** CardPreviewServiceTest, CardImageCompositor. **재사용:** 기존 nullable asset/text 처리. **완료 조건:** 학교 직인이나 한자가 없는 합법적 입력에서 해당 영역만 비어 있고 Preview 전체가 실패하지 않는다. **✅ 한자 없음 케이스만 완료** — `composesStudentBackWithoutHanjaVariant`. "직인"은 STUDENT엔 해당 없음(로고·직인 자체를 렌더링하지 않는 카드종류 — `validateIssuerAssets`에 `cardType != STUDENT` 가드 추가한 것도 이 때문, 4-C 버그 1번).
- [ ] **Preview 무저장 불변조건 확인** — **확인 대상:** CardPreviewServiceTest, StorageService mock/spy. **재사용:** 기존 read-only Preview. **완료 조건:** 호출 전후 Application/Member/UploadFile row와 S3 객체 수가 같고 다운로드 외 업로드·삭제가 없다. **⚠️ 미완료** — 이 불변조건은 cardType과 무관하게 `CardPreviewService` 자체가 이미 보장하고 다른 카드종류로 테스트돼 있지만, STUDENT 경로(템플릿 다운로드 추가)로 다시 태워 재확인한 자동 테스트는 없다.
- [ ] **관리자 프론트 연결 작업 분리 기록** — **확인 대상:** frontend/src/services/api.ts, ApplicationsSection.tsx, docs/FRONTEND_API_GAPS.md. **재사용:** 기존 관리자 신청 상세 화면. **완료 조건:** 디자인 목록, 발급일자, Member 선택, FRONT/BACK Preview 호출이 없으면 Frontend NOT CONNECTED로 기록하며 백엔드 결과를 FAIL 처리하지 않는다. **미착수** — 프론트 스코프, 이번 세션 대상 아님(그대로 유지).

**2-B 테스트:** CardPreviewServiceTest → CardImageCompositorTest → 관련 AdminApplicationControllerTest → 학생증 4조합 앞·뒷면 시각 산출물 검증 → compileJava. 탐색용 StudentCardExploratoryRenderTest는 정식 fixture·assertion으로 승격한 경우에만 자동 테스트 근거로 인정한다. **⚠️ 부분 완료(2026-08-31)** — `CardImageCompositorTest`(STUDENT 6종 추가)와 4조합 시각 검증은 완료. `CardPreviewServiceTest`/`AdminApplicationControllerTest`는 STUDENT 케이스가 아직 없음(위 두 미완료 항목과 동일) — 다음 작업(4-D 이후 또는 그 전이라도)에서 채워야 이 줄 전체가 완료된다. `StudentCardExploratoryRenderTest`는 여전히 승격 안 됨(다른 세션 소유 파일, 이번에도 안 건드림).
