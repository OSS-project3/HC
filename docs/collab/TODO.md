...
3. **자동 생성되는 `CardDesign`의 `name`/`isDefault` 값** — 업로드 API가 신규 생성할 때 `CardDesign.name`을 뭘로 채울지(예: `"{School.name} {orientation}"`), `isDefault`는 항상 `true`로 둘지(학교당 방향별 디자인이 어차피 1개뿐이라 의미가 옅음) 미정.
