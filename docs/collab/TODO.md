...
## 관리자 상태 전이 — 드롭다운 제거, 업무 버튼 + 자동 전이 (2026-09-24 정책 확정, 미착수)

상태: ⚪ 대기. 정책 확정, 백엔드부터 구현 예정.
...
- [ ] `ApplicationDetail.tsx`의 "상태 변경" `<select>`를 제거하고, 남는 업무 액션(입금확인/검토시작/작명승인/사진반려/카드발급완료/배송발송)을 각각 독립 버튼으로 전환.
- [ ] "작명완료 처리"/"제작 시작" 항목은 목록에서 제거(백엔드가 자동 처리).
- [ ] 전체 파이프라인 단계를 보여주는 토글/드롭다운 형태의 읽기 전용 상태 표시기 신설.
- [ ] 자동 전이가 발생했을 때 관리자에게 어떻게 알릴지(토스트 등) 결정 — 아직 미정.
...
|---|---|---|---|---|---|
| ⚪ | 관리자 상태 전이 — 드롭다운 제거, 업무 버튼 + 자동 전이 | 미정 | `main` | 본 문서 "관리자 상태 전이 — 드롭다운 제거, 업무 버튼 + 자동 전이" 절 | 정책 확정(이름확정/카드생성 버튼은 그대로, 마지막 멤버 완료 시 자동 전이), 구현 미착수 |
