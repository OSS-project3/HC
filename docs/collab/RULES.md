...
- [ ] 변경 내용이 아래 5가지 기준과 일치하는지 검증
  - [ ] 해당 도메인 `requirements.md`(있으면)와 일치하는가
  - [ ] 해당 도메인 `data-model.md` 구조와 일치하는가
  - [ ] `docs/api/common.md` 공통 규칙을 준수하는가
  - [ ] 기존 API/문서와 충돌하지 않는가
  - [ ] **모듈 경계·Bean 배치·인증 방식 등 구조적 결정을 새로 내렸다면 `arch.md`에도 반영했는가** (2026-08-19 추가 — 4순위 "참고 문서"라고 건너뛰지 않는다, `arch.md` §18 참고)
...
**작성 순서(TDD)**: 가능하면 테스트를 먼저 작성해 의도한 대로 실패하는지 확인한 뒤 최소 구현을 추가하고, 관련 테스트 스위트를 재실행해 통과를 확인한다.

### 작업 단위별 테스트 범위 (2026-08-19 신규 — Inquiry 도메인 구현 중 명문화)

- **각 작업 단위(예: 엔드포인트 1개, 도메인 내부 메서드 1개)에서는 변경된 기능의 직접 테스트와, 영향받는 공통 영역(예: `SecurityConfig`, 공용 예외 처리, 공용 DTO)의 관련 테스트만 먼저 실행한다** — `--tests`로 범위를 좁혀서 돌린다(예: `--tests "com.example.honorcitizen.domain.inquiry.*"`).
- 단순 도메인 내부 변경(같은 도메인 안에서 메서드·필드 하나 추가하는 수준)마다 전체 테스트 스위트를 반복 실행하지 않는다 — 실행 시간 낭비이자 로그 과다 출력의 원인이다.
- 각 단위를 커밋하기 전 `git status`/`git diff`로 **예상 밖의 변경 파일이 없는지 확인**한다 — 의도한 파일만 스테이징됐는지, 다른 단위(예: 이후 착수할 INQUIRY-4)에 속한 파일이 실수로 섞이지 않았는지를 커밋 직전 항상 점검한다.
- **전체 테스트 스위트(`./gradlew test`, 인자 없이)는 아래 경우에만 실행한다:**
  - 여러 작업 단위로 이뤄진 기능 묶음이 전부 끝났을 때(예: 체크리스트 전 단위 완료 후)
  - `SecurityConfig`, 공용 예외 처리(`GlobalExceptionHandler`), 인증/인가 등 여러 도메인에 영향을 주는 공통 인프라를 변경했을 때
  - `push`/PR 직전 최종 회귀 검증
- 전체 테스트 스위트를 실행할 때는 §9 규칙대로 로그를 파일로 리다이렉트하고, **종료 코드·전체 테스트 수·신규 실패 테스트만** 우선 확인한다(기존에 알려진 pre-existing 실패와 구분할 것 — 새 실패인지 먼저 판단).
